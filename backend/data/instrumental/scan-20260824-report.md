...
| 級 | 首數 | 去向 |
|---|---|---|
| 實錘(佔位符型 whisper) | **69** | T3 自動落 `instrumental=1` |
| 已知名單補入(歌名/SUPERVISION-LOG 有獨立證據) | **3** | T3 自動落 `instrumental=1` |
| **T3 回標總數** | **72** | `apply-20260824.json` |
| 擦邊 | **199** | 人手名單,等 Eric 過目,**唔寫 DB**(其中 6 首歌名有器樂訊號 = 最高優先,見 §3.1) |
| whisper 標到人聲/觀眾聲 | **212** | 判定唔係器樂,唔入名單(§6) |
| verified 但 whisper 全程靜(觀察) | **811** | **唔准動** |

## §2 實錘名單(69 首,佔位符型 whisper)
...
## §3 擦邊名單(199 首)—— 唔寫 DB,等 Eric 過目

_(原始 soft 判定 202 首,扣走 3 首經 §5 已知名單補入咗 T3 回標嘅。)_

### §3.1 最高優先:歌名有器樂訊號,只差 whisper 實證(6 首)
...
| 3225 | 【恩典之路】鋼琴版 | 天弦音樂事工 | 3:26 | unavailable | 13/5 | 93% | 27/0 | — | 詞李宗盛曲李宗盛 ⏎ 詞李宗盛 ⏎ 編曲李宗盛 ⏎ 混音李宗盛 ⏎ 監製李宗盛 |
| 3226 | 【戒指】鋼琴版 | 天弦音樂事工 | 3:32 | unavailable | 8/4 | 100% | 32/0 | — | 詞曲李宗盛編曲李宗盛 ⏎ 歌曲李宗盛 ⏎ 詞曲李宗盛 ⏎ 你不想要我做你的女朋友吗 |
...
| 3230 | 【不只是浪漫】鋼琴版 | 天弦音樂事工 | 4:01 | unavailable | 9/3 | 101% | 29/0 | — | 詞曲李宗盛編曲李宗盛 ⏎ 詞曲李宗盛 ⏎ 我就是想要你做我的朋友 ⏎ 知道嗎 |
...
_(表只列頭 50 首,全份 811 首喺 `scan-20260824.json` 嘅 `observe` 陣列。呢個數字大係正常:whisper 幻覺 loop 喺全庫好普遍,而呢啲歌係靠 OCR 做到 verified 嘅。)_
...
已知名單 **42** 首入面,**36** 首入咗 T3 回標(已 dedupe),**6** 首唔入(原因見上表)。

其中 **3** 首係「whisper 判唔到(幻覺型 / 冇 timeline)但已知名單有獨立證據」而補入嘅:
