...
| 級 | 首數 | 去向 |
|---|---|---|
| 實錘(佔位符型 whisper) | **75** | T3 自動落 `instrumental=1` |
| 已知名單補入(歌名/SUPERVISION-LOG 有獨立證據) | **13** | T3 自動落 `instrumental=1` |
| **T3 回標總數** | **88** | `apply-20260824b.json` |
| 擦邊 | **193** | 人手名單,等 Eric 過目,**唔寫 DB**(其中 10 首歌名有器樂訊號 = 最高優先,見 §3.1) |
| whisper 標到人聲/觀眾聲 | **212** | 判定唔係器樂,唔入名單(§6) |
| verified 但 whisper 全程靜(觀察) | **811** | **唔准動** |

## §2 實錘名單(75 首,佔位符型 whisper)
...
## §3 擦邊名單(193 首)—— 唔寫 DB,等 Eric 過目

_(原始 soft 判定 206 首,扣走 13 首經 §5 已知名單補入咗 T3 回標嘅。)_

### §3.1 最高優先:歌名有器樂訊號,只差 whisper 實證(10 首)
...
_(表只列頭 50 首,全份 811 首喺 `scan-20260824b.json` 嘅 `observe` 陣列。呢個數字大係正常:whisper 幻覺 loop 喺全庫好普遍,而呢啲歌係靠 OCR 做到 verified 嘅。)_
