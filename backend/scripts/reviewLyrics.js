...
//                                     唔使人手手動改 DB。
//                {id, unusable:true} — 2026-08-13 加(清殭屍):人手讀過個 draft,判斷
//                                     底本本身冇得救(OCR 徹底亂碼/whisper 幻覺佔位符/
//                                     語言標錯韓日台語/純器樂冇歌詞/medley 冇法拆),
//                                     標 lyrics_status='unavailable' 徹底踢出複核隊列。
//                                     ⚠️ 同 demote 嘅分別:demote 係「拉返落嚟再校」
//                                     (下輪 export 會再出現);unusable 係終態,export
//                                     同 fetchLyrics 候選都唔會再見到佢。lyrics_draft
//                                     保留唔剷,第日想翻案可以人手改返 status='draft'。
//                                     ⚠️ 非歌內容(訪問/教學/花絮)唔好用呢個——嗰啲
//                                     跟返 delist 流程(lib/adminHymns.js delistHymn)。
//
...
async function doApply(file) {
  ...
  if (!Array.isArray(items)) { log('檔案要係 [{id, lyrics}] / [{id, demote:true}] / [{id, unusable:true}] 陣列'); process.exit(1); }

  let applied = 0, demoted = 0, unusabled = 0, skipped = 0;
  for (const item of items) {
    const id = item?.id;
    if (!id) { skipped++; continue; }

    // 2026-08-13 加:unusable —— 人手讀過,判斷底本冇得救(亂碼/語言標錯/純器樂),
    // 標 unavailable 徹底踢出隊列。lyrics_draft 保留做證據,想翻案人手改返 draft。
    if (item?.unusable === true) {
      ...
      try {
        ...
        freshDb.run(
          `UPDATE hymns_all SET lyrics=NULL, lyrics_status='unavailable', lyrics_checked_at=? WHERE id=?`,
          [today(), id]
        );
        saveDb(freshDb);
        unusabled++;
  ...
  log(`--apply 完成:寫入(verified)${applied} 首,demote(退返draft)${demoted} 首,unusable(踢出隊列)${unusabled} 首,skip(未校/冇 lyrics/冇 id)${skipped} 首`);
}
