#!/usr/bin/env node
// backfillAlbumFromKeenCatalog.js — ALBUM-BACKFILL-ACCEL-PLAN.md Phase B,
// 「基恩敬拜 Amazing Grace Worship(AGWMM)」版。食
// `data/album-backfill/keen-catalog.json`(scrape 自官網 agwmm.org ~48 個
// `/product/album-*/` 商品頁:歌名+所屬專輯+年份),對 org='基恩敬拜' 冇
// album 嘅 row 做歌名 match,match 到先寫。
//
// ── DB 標題嘅特殊之處 ──────────────────────────────────────────────────
//   基恩敬拜 DB 嘅 title/display_title 多數用《中文歌名》包住,後面跟
//   English Title 同埋「現場版/AGWMM/Official MV/…」呢類標籤,例如
//   「《祢是我惟一的倚靠》I Put My Trust In You (現場版)｜ 禱告更新2025｜
//   AGWMM Official Live」。少數 display_title 已經淨返純歌名(例如
//   「找到方向」)。所以呢度**先攞《》/【】/[] 入面嘅內容**做候選歌名,
//   冇括號就用成句(或 " - " 前嗰部份)做候選,再用 "/" 切中英,先做
//   normalize 比對(cn→tw、全半形、去空格)——同 backfillAlbumFromJoshuaCatalog.js
//   同一套邏輯,淨係括號 regex 加咗 《》。
//
// ── 匹配規則(寧空莫錯,跟 backfillAlbumFromCatalog.js/Joshua 版同一套)──
//   · normalize 後嘅候選歌名(中文或英文)撞唔到 catalog → 唔寫
//   · 撞到但對應多過一隻唔同專輯(同名歌喺唔同專輯出現,例如 128/124
//     呢類精選碟重複收錄舊歌)→ 唔寫,flag 落 report 人手覆核
//   · 撞到得一隻專輯,而且 DB 呢個 row album 本身空、album_source 唔係
//     manual/legacy → 寫 `album=<catalog 專輯名>, album_source='website'`
//
// Usage:
//   node scripts/backfillAlbumFromKeenCatalog.js --dry   # 出 report,唔寫 DB
//   node scripts/backfillAlbumFromKeenCatalog.js         # 真寫
...
async function main() {
  ...
  if (!DRY && matched.length) {
    const token = await acquireDbLock('backfillAlbumFromKeenCatalog');
...
function writeReport({ rows, matched, conflicts, alreadyHasAlbum, protectedRows, notFound, dry }) {
  const lines = [];
  lines.push('# backfillAlbumFromKeenCatalog 報告 —— Phase B(agwmm.org 官網 catalog)');
  lines.push('');
...
main().catch((e) => { console.error('backfillAlbumFromKeenCatalog 出錯:', e); process.exit(1); });
