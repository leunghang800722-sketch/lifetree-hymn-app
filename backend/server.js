...
app.get('/api/audio/:youtubeId', async (req, res) => {
  ...
  try {
    ...
    const url = execSync(
      `yt-dlp -f "bestaudio[ext=m4a]/bestaudio" --cookies cookies.txt --get-url --no-playlist "https://www.youtube.com/watch?v=${youtubeId}"`,
      { encoding: 'utf8', timeout: 30000 }
    ).trim();

    // Get metadata (title + duration)
    const metadataJson = execSync(
      `yt-dlp --cookies cookies.txt --print-json --no-playlist "https://www.youtube.com/watch?v=${youtubeId}"`,
      { encoding: 'utf8', timeout: 30000 }
    ).trim();
    ...
    try {
      const urlFallback = execSync(
        `yt-dlp -f "bestaudio" --cookies cookies.txt --get-url --no-playlist "https://www.youtube.com/watch?v=${youtubeId}"`,
        { encoding: 'utf8', timeout: 30000 }
      ).trim();
