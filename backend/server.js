...
app.get('/api/audio/:youtubeId', async (req, res) => {
  ...
  try {
    ...
    const url = execSync(
      `yt-dlp -f "bestaudio[ext=m4a]" --cookies cookies.txt --get-url --no-playlist "https://www.youtube.com/watch?v=${youtubeId}"`,
    ...
    try {
      const urlFallback = execSync(
        `yt-dlp -f "bestaudio/best" --cookies cookies.txt --get-url --no-playlist "https://www.youtube.com/watch?v=${youtubeId}"`,
        { encoding: 'utf8', timeout: 30000 }
      ).trim();
