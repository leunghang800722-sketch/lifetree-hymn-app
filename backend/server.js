...
app.get('/api/audio/:youtubeId', async (req, res) => {
  ...
  try {
    ...
    const url = execSync(
      `yt-dlp -f "bestaudio[ext=m4a]" --cookies cookies.txt --js-runtime deno --get-url --no-playlist "https://www.youtube.com/watch?v=${youtubeId}"`,
      { encoding: 'utf8', timeout: 30000 }
    ).trim();

    // Get metadata (title + duration)
    const metadataJson = execSync(
      `yt-dlp --cookies cookies.txt --js-runtime deno --print-json --no-playlist "https://www.youtube.com/watch?v=${youtubeId}"`,
      { encoding: 'utf8', timeout: 30000 }
    ).trim();
    ...
    try {
      const urlFallback = execSync(
        `yt-dlp -f "bestaudio/best" --cookies cookies.txt --js-runtime deno --get-url --no-playlist "https://www.youtube.com/watch?v=${youtubeId}"`,
        { encoding: 'utf8', timeout: 30000 }
      ).trim();
