...
app.get('/api/audio/:youtubeId', async (req, res) => {
  ...
  try {
    ...
    const url = execSync(
      `yt-dlp -f "bestaudio[ext=m4a]" --get-url --no-playlist --extractor-args "youtube:player_client=android" "https://www.youtube.com/watch?v=${youtubeId}"`,
      { encoding: 'utf8', timeout: 30000 }
    ).trim();

    // Get metadata (title + duration)
    const metadataJson = execSync(
      `yt-dlp --print-json --no-playlist --extractor-args "youtube:player_client=android" "https://www.youtube.com/watch?v=${youtubeId}"`,
      { encoding: 'utf8', timeout: 30000 }
    ).trim();
    ...
    try {
      const urlFallback = execSync(
        `yt-dlp -f "bestaudio/best" --get-url --no-playlist --extractor-args "youtube:player_client=android" "https://www.youtube.com/watch?v=${youtubeId}"`,
        { encoding: 'utf8', timeout: 30000 }
      ).trim();
