...
app.get('/api/audio/:youtubeId', async (req, res) => {
  ...
  try {
    ...
    const metadataJson = execSync(
      `yt-dlp --print-json --no-playlist "https://www.youtube.com/watch?v=${youtubeId}"`,
      { encoding: 'utf8', timeout: 30000 }
    ).trim();
    ...
    try {
      const urlFallback = execSync(
        `yt-dlp -f "bestaudio/best" --get-url --no-playlist "https://www.youtube.com/watch?v=${youtubeId}"`,
        { encoding: 'utf8', timeout: 30000 }
      ).trim();
