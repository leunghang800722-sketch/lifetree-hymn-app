...
- [2026-08-19 13:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+27**(log累計 876);重做隊剩 81;可做draft 108;producer 行緊
- **心跳 2026-08-19 14:01** — verified 3378(54.2%)/ draft 143(可做 73)/ 重做隊剩 115 / producer 生存(keeper pid 8368、OCR pid 8425)/ ⚠️ /tmp/hymn_fetchlyrics.log 最後一行停喺 06:00 但 mtime 14:00,log 寫入 offset 壞咗(疑似兩個 writer 共用 fd),唔好再靠呢個 log 判斷進度
- [2026-08-19 14:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+25**(log累計 901);重做隊剩 56;可做draft 132;producer 行緊
- [2026-08-19 15:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+34**(log累計 935);重做隊剩 22;可做draft 165;producer 行緊
- [2026-08-19 16:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+46**(log累計 981);重做隊剩 15;可做draft 210;producer 行緊
- **心跳 2026-08-19 17:01** — verified 3388(54.4%)/ draft 250(可做 222)/ 重做隊剩 15 / producer 生存 / draft池4個鐘由132升到222但複核線冇消化,verified自12:00起無升,積壓中
- [2026-08-19 17:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+54**(log累計 1035);重做隊剩 15;可做draft 114;producer 行緊
- [2026-08-19 18:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+60**(log累計 1095);重做隊剩 15;可做draft 172;producer 行緊
- [2026-08-19 19:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+59**(log累計 1154);重做隊剩 15;可做draft 228;producer 行緊
- **心跳 2026-08-19 20:01** — verified 3527(56.6%)/ draft 288(可做 253)/ 重做隊剩 15 / producer 生存 / draft 可做量由 19:37 嘅 228 升到 253,producer 每鐘出約 +59 但 reviewer 線追唔切,積壓緩慢上升
- [2026-08-19 20:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+56**(log累計 1210);重做隊剩 15;可做draft 282;producer 行緊
- [2026-08-19 21:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+60**(log累計 1270);重做隊剩 15;可做draft 229;producer 行緊

## 2026-08-19 21:48 — R1 國語線(第二班,20:52–21:48)

**數字:** DB verified 3527 → **3686**(當中 R1 寫入 127 首,其餘係 R2 同步進行)。全庫可做 draft 開波 299 → 收爐 201。
**決定分佈:** 182 個決定 = **verified 127 / 留 draft 55 / unusable 0 / delist 0**。audit 11 批共 130 條入面 reject 1(7880 太薄)、langmismatch **0**。11 批 apply 全部覆查零 race。**WebSearch 3/4 次**(全部用喺 300 投靠者的讚美 同 1913 不再保留,只確認到「哪一個施恩者像祢」一項,其餘兩查搵唔到內文)。producer keeper 全程生存(pid 8368)。串流健康零警報。

**⛔ 最要緊嗰件事 —— restart 押後,127 首未出街:**
班尾 checkpoint 行到 approve 前停低。`[stream]` 靜音檢查過關(最後播放 19:13,靜 2.5 鐘),launchctl 7 個 job 正常,但 `approved sha..HEAD` 有 **16 個未 approve commit,其中 4 個係另一條 session 喺 21:37–21:45(我做緊嘢嗰陣)先寫好嘅 backend 串流改動**(5753fff / 7377bd6 / 20dcf7c / da191f0,涉及 buffered fast-path、/warm 讓路、冷路徑 tee bufferCache、06:30 預 resolve cron)。照 SOP §5-2「見到唔明嘅其他 session backend commit 就 skip restart」押後。**已 curl 實測確認 live `/api/hymns` 仲未吐今班歌詞**(234 / 6879 / 8535 / 7929 四首都係空)。下一轉 R1 開波第一件事應該係處理呢單。

**方法論收穫(建議寫入 SOP,四項):**
1. **whisper 補 OCR 缺口嘅可覆核界線** —— whisper 喺缺口位讀得清、冇亂碼 → 可以補(實例 3188 唯獨耶穌補返副歌主語、3156 用我一生補「向標竿奔往」、234 我安然居住補結句、8078 補「罪中沉淪」);whisper 本身 garbled 要靠估 → 唔補照留 draft(反例 1913 / 300 / 8040)。
2. **同名歌兩條片互補 —— 今班奏效四次**(234↔7502 我安然居住、7596↔7649 風和愛、7929→7708 歡呼收割、5880↔42 榮耀大君王)。建議每班開波先掃一次分區入面嘅重複 title,已 verified 嘅直接沿用。
3. **dedupe 會食走「多段共用嘅重複句」** —— uniq 少咗唔一定係 OCR 漏。實例 7607 我不求(三段共用「我不敢祈求/只求祢」)、8632 一路靠著祂、6373 親愛的要記得。還原重複句唔算加內容。
4. **重讀 draft 前先 diff 底本** —— 我第二次 export 之後逐首 diff,確認 11 首留低嘅 draft 底本零更新,直接慳返 11 個決定唔重判。

**新記低嘅 OCR 病(三個):**「祢」認成「**杯**」(礻→木、尔→不,實例 4924);整段英文行嘅「**a**」全部認成「**o**」(實例 7098,單一字母系統性替換,逐個改返安全);whisper 幻覺 spam 行(實例 334 尾段出「請不吝點贊訂閱轉發打賞…」,唔好當歌詞)。

**vein 判斷(重要更正):**
- 🟢 **高產**:讚美之泉官方歌詞版 MV、約書亞樂團/大衛帳幕的榮耀(官方歌詞MV + 官方KALA版)、小羊詩歌中英字幕、**泥土音樂/盛曉玫**(⚠️舊「死症」標籤確認過時)、天韻 **Official MV / 字幕版 / 動態歌詞版** 系列。
- 🔴 **結構性冇救(建議 P 線唔好再排 OCR)**:天韻**舊碟片**(創作專輯之二/之五/之八/之十/之十一)—— 浮水印蓋死字幕;天韻**靜態專輯封面片** —— **實錘證據:7769 祢是我一切 同 7773 我的眼睛已經看見 係兩首唔同嘅歌但 OCR 出嘅內容一模一樣**,同碟每首互相污染。分區入面呢類仲有約 28 條。
- 🔴 **讚美之泉兒童系列短歌冇 shortOk 出路**:6371 / 6385 兩單 whisper 都係全程 [MUSIC],實證唔到,唔好再逐首試。

**要 Eric 拍板嘅三件事:**
1. **whisper-only 通順個案出唔出?** 實例 **7728 等你願意** —— whisper 由頭到尾通順零亂碼,但單一來源、而且「它/祂/他」分唔到而成首歌靠呢個字。我冇出。同類仲有 7732 傾聽、7741 自從我認識耶穌。
2. **簡體 / 外語段點處理?** 8618 渴慕與尋求(我心旋律)我**轉咗簡體做繁體**、**淨出中文唔出希伯來文**;7398 奇異恩典我**剷咗成段唔完整嘅日文**;6995 耶穌我愛祢到永遠我**連台語段一齊出**。四個都係我自己判,要覆核。
3. **4 首台語歌 lang 標錯做「國語」**(唔屬我職權冇改):**7698 啥事你冷心倒退(台)、8012 阿母的頭毛、8082 走馬燈、8067 呣免驚耶穌在此**。

**Fable 5 抽查名單(今班新 verify 抽 12 首):** 234 我安然居住 / 334 我的守護者 / 3188 唯獨耶穌 / 3959 神啊祢在何處(大提琴演奏,器樂版上歌詞判例) / 6373 親愛的要記得 / 6937 在地如在天 / 7364 只憑基督誇口 / 7502 我安然居住(KALA版) / 7702 表達 / 7929 歡呼收割 / 8012 阿母的頭毛(台語) / 8618 渴慕與尋求(簡轉繁 + 剷希伯來文)。


## 2026-08-19 21:50 — 每日自動歌詞校對(lyrics-daily-proofread):幾乎冇工可做,只做咗 1 首

**結論先講:呢個 routine 同三線平行複核撞晒位,今晚可做嘅只有 1 首(台語孤兒),已完成。**

### 現況數字
- `reviewLyrics.js --export` → **247 首 draft**;過 `bi-freeze` BI 濾網後 **可做 210 / 扣起 37**
- 可做 210 首分區:**國語 121 / 英文 72 / 粵語 16 / 台語 1**
- 全庫:verified **3687** / draft 493 / unavailable 706 / none 3280(20:01 心跳時 verified 係 3527,即 R 線兩個鐘做咗 +160)

### 點解冇工可做 —— 四個分區全部唔到我手
| 分區 | 數量 | 狀況 |
|---|---|---|
| 國語 | 121 | **R1 `lyrics-line-mandarin` 行緊**,ledger 21:38→22:17 每 7 分鐘一批 17–19 個決定,火力全開 |
| 粵語 | 16 | **R2 `lyrics-line-cantonese` 行緊**(最後 21:24) |
| 英文 | 72 | **Eric 2026-08-19 拍板「暫緩-版權」**,`lyrics-copyright-hold.json` 註明 draft 保留唔郁:唔 apply / 唔判 unusable / 唔 delist |
| 兒童 | 2 | 兩首都俾 BI 濾網扣起 |
| 台語 | 1 | ✅ **冇任何線認領,我做咗** |

`ops/lyrics/REVIEW-LINE-SOP.md` §1.1 寫明「做唔屬於你分區嘅歌 = 直接違規,會同隔籬線撞單、白燒兩份額度」,
所以國語/粵語**一首都冇掂**。英文分區更加係 Eric 明文封存,亦冇掂。

### 今晚唯一一首:7872【投靠者的謳咾】天韻合唱團(台語)
- OCR 底本結構完整(兩段主歌 + 副歌),噪音係:逐幀跳出嘅「天」字浮水印、簡體經文卡、`经文摘编：葉藏心` credit 行、称/祢/你 三種寫法混住。
- **WebSearch 核對過 1 次**(全日 budget 30,用咗 1):Mojim 收錄嘅正本確認咗「叨一位君王親像祢」同「祢的信實佇祢的四圍,看輕祢的見羞」——
  即係 draft 嘅 `叼` 應作 **叨**、`称/你` 統一做 **祢**、標題 `謳佬` 應作 **謳咾**(同 DB title 一致)。逐字正返,冇照抄第三方文字。
- ⚠️ **保留咗一個唔肯定嘅詞**:副歌「**嗨**管風浪凶惡罩來」,三幀 OCR 一致但唔似台語正字(疑係「毋管」之類)。
  按「爭一句唔好扣起成首」嘅做法照原樣出街,**冇亂改亦冇亂猜**,第日搵到正本可以再修。
- audit **1/1 pass**(reject 0、langmismatch 0)→ apply 寫入 verified → 隔 20 秒覆查 `lyrics_status=verified`、length 159,**冇中 producer race**。
...
- **7872 【投靠者的謳咾】天韻合唱團 Official MV(台語)**

### 建議俾 Eric(未做,等拍板)
呢個 `lyrics-daily-proofread` routine 而家嘅實際價值 ≈ 零 —— 三條 R 線 + 版權封存已經冚晒全部分區,
佢每日行一次淨係做重複 export、燒少少額度,最壞情況係手快過 R 線撞單。
**建議直接 disable 佢**,等三線 SOP 做唯一入口;或者改造成「孤兒分區清道夫」(專做冇線認領嘅 lang,例如今次嘅台語)。
- [2026-08-19 22:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+49**(log累計 1319);重做隊剩 14;可做draft 237;producer 行緊
- **心跳 2026-08-19 23:01** — verified 3677(59.0%)/ draft 283(可做 245)/ 重做隊剩 5 / producer 生存(keeper pid 8368)/ 三個鐘 verified +150、重做隊 15→5 收窄緊,draft 可做量由 253 微跌到 245,積壓終於企穩;fetchLyrics log 最後一行仍停喺 15:00(已知 offset 壞,唔好靠佢判斷)
- [2026-08-19 23:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+32**(log累計 1351);重做隊剩 0;可做draft 268;producer 行緊
- [2026-08-20 00:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+57**(log累計 1408);重做隊剩 0;可做draft 325;producer 行緊
- [2026-08-20 01:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+37**(log累計 1445);重做隊剩 0;可做draft 242;producer 行緊

## 2026-08-20 01:39 — R1 國語線(第三班,凌晨 00:52–01:39)

**DB 數字:** verified 3686 → **3794**(+108,含上一班積壓 127 一齊放出街);draft 池 274;分區(國語)剩 draft 155;全庫可做 235。live `/api/hymns` 覆蓋率 **60.9%**(6230 首中 3794 首有歌詞)。

**今班產出:** 193 個決定 / 47 分鐘,九批。verified **91** / unusable **2**(3989 與你同走過大提琴演奏、5988 注目看耶穌安靜系列 —— 兩首都係實錘零字幕零人聲)/ delist **3** / 留 draft **97** / audit reject **1**(6026,太薄且 whisper 空,實證唔到 shortOk)/ langmismatch **0** / hold **0**。九批覆查全部零 race。WebSearch **1/4**。

**方法論(呢班最主要嘅產出,建議下一班照抄):**

1. 🏆 **「同名孖仔」掃描 + whisper 身份核對** —— 單靠呢招今班打出 **14 首**,成本近乎零。做法:draft 標題正規化(抽【】內容、剷官方/歌詞/動態/版/MV/Official/Jazz ver./敬拜/團名等修飾詞、剷英文標點)→ 撞 `lang='國語'` 已 verified 池 → **逐對攞 draft 自己條片嘅 whisper 對返孖仔歌詞** 先落實。掃出 21 對,**篩走 7 對**(最重要嗰個反例:7716 天韻《聖靈的果子》撞到 5890 讚美之泉兒童《聖靈的果子》,**兩首完全唔同作品**,齋靠標題會寫錯成首歌)。腳本喺我 scratchpad `dup.mjs` / `cmp.mjs`,邏輯已寫入 ledger。
2. **卡住一首歌之前,先搵庫入面有冇同名第二條片** —— 比 WebSearch 平、比 whisper 準。實例:300【投靠者的讚美】卡咗兩班、燒咗兩次 WebSearch,今班喺天韻分區撞到 7659 同名完整版,一次過證實嗰句係「**屬祢**的人大有平安/保障」而唔係大家一直估嘅「投靠的人」。
3. **whisper 三種用法要分清** —— (a)**補缺口**:whisper 喺缺口位清晰冇亂碼先可以補(8241 弟兄和睦同居、8023 歌頌復活主、5416 我一生要讚美祢);(b)**探測缺口**:OCR 睇落乾淨完整,whisper 一查就見到成段冇捕捉到嘅副歌(4024/4066/3474/3966/3665 五首因此留 draft,後尾 4024/4066 靠孖仔救返);(c)**認歌**:whisper 亂到唔可以抄,但夠用嚟確認「係咪同一首歌」(批7 四對孖仔)。
4. **kinetic typography 片**(逐個詞飛入)—— OCR 有字冇斷行、whisper 有斷行冇字,兩邊夾埋可以出街(4914 大聲敬拜、6813 這是我異象)。
5. **一首歌兩條片,其中一條有雜訊** —— 用乾淨嗰條分辨邊句係歌詞(7321 vs 7491 現場醫治特會版夾住「請得醫治的病人 get checked by the staff」公告字幕)。

**⚠️ 要 Eric / P 線拍板嘅事(按重要性):**

1. 🔴 **天韻合唱團「舊碟靜態封面版」約 90 首,結構上做唔到。** 條片**本身冇歌詞字幕**(OCR 由頭到尾淨係讀緊封面浮水印「天韻創作專輯之X」「與仁仙有約」等,每張碟一組固定亂碼),再點行 OCR 都係零。**但 whisper 有嘢**(抽驗 7676/7759/7769 都有完整段落),所以**唔應該判 unusable**。建議:另開 whisper-first 路數處理,或者暫時標記移出複核隊列 —— 佢哋而家每輪 export 都佔位,係空轉。
2. **「draft = 一大段 whisper 全文」呢種新型底本要小心。** 天韻池入面好多(7671/7674/7716/7732/7741/7763/7811/7868/7902/7917…),**讀落成段通順,但夾住致命同音錯**(「生靈結的果子」=聖靈、「我要乘送野和荒」=稱頌耶和華、「讚美的花瓶在我口中」=話語、「玩具的心你要遠離」、「耶穌不是神明的光」=生命)。今班一句都冇抄。
3. **7611【發現】天韻 Jazz ver. 出咗街嘅歌詞本身殘缺**(開頭「的蹤跡何其難尋」甩字、結尾「我的心有祢 深深倚」斷喺半句),建議 demote 重校。
4. **metadata `lang` 欄標錯,要人改:** 8179 如若(實際粵語)、8262 詩篇二十三篇 / 8263 我欲等候耶和華 / 7062 我主耶穌真好國台雙語 / 8085 將伊藏在心肝底(實際台語)、8428 愛を形に(實際日語)。**台語歌我校對唔到**(唔敢決定「謳佬/謳咾」「袜/袂」「精英/甦醒」邊個啱),全部留 draft 等識台語嘅人。
5. **1362 The King of Christmas 係一齣雙語聖誕音樂劇唔係一首歌**(一半 OCR 係對白),**唔算「明確非歌內容」所以我冇自行 delist**,模糊個案留 draft 等拍板。
6. **6385 賜福與你 vs 6199 同名但歌詞真係唔同版本**(6385 唱「恩典降下賜福與你」、6199 寫「平安喜樂 天天充滿你」),冇夾硬套。

**落架 3 首(明確非歌內容,已按 Eric 2026-08-09 標準自行處理):** 2161 無盡感恩話香港(廿二年服事回顧紀錄片)、6539 滿有能力(電視訪談節目片段)、7821 Making Heavenly Music【真相】(幕後製作特輯)。

**班尾 checkpoint:** `[stream]` 靜音過關(最後活動係 healthcheck 三條固定 id,同 6 鐘前嗰組對得上)→ approve `da191f0`(上一班押後嗰四個串流 commit,HEAD 四個鐘冇郁、源碼零未 commit 改動,符合上一班寫低嘅出路 b)→ restart health OK → `launchctl | grep hymnapp` = 7 → live 抽驗 7659/8361/4066 三首吐到歌詞,**另核實上一班 127 首積壓(234/6879/8535/7929)而家全部出街**。串流健康零警報。

**抽查名單(今班新 verify 隨機 8 首,俾覆核線核):** 366 悅納的時候、1538 不要為明天憂慮、4914 大聲敬拜、6852 井邊、7544 我屈膝、7659 投靠者的讚美、7916 野地的花、8241 弟兄和睦同居。
- **心跳 2026-08-20 02:01** — verified 3802(61.1%)/ draft 274(可做 234)/ 重做隊剩 0 / producer 生存(keeper pid 8368)/ 三個鐘 verified +125,重做隊 5→0 清晒;keeper 22:32→01:27 中間近 3 個鐘冇開新批(池由 1458 跌到 1338),draft 可做量 245→234 慢慢消化中;fetchLyrics log 最後一行停喺 18:01(已知 offset 壞,唔好靠佢判斷)
- [2026-08-20 02:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+41**(log累計 1486);重做隊剩 0;可做draft 262;producer 行緊
- [2026-08-20 03:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+47**(log累計 1533);重做隊剩 0;可做draft 309;producer 行緊
- [2026-08-20 04:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+32**(log累計 1565);重做隊剩 0;可做draft 335;producer 行緊
- **心跳 2026-08-20 05:01** — verified 3802(61.1%)/ draft 394(可做 329)/ 重做隊剩 0 / producer 生存(keeper pid 8368,04:27 開咗 OCR budget 120)/ ⚠ verified 由 02:01 起三個鐘紋風不動(一樣係 3802),但 draft 274→394、可做 234→329 一路積,即係 P線出緊貨而複核線(R1/R2/R3)似乎冇喺度消化,建議 Eric 睇下複核班係咪死咗;fetchLyrics log offset 壞嘅老問題仍在(尾段係 NUL,文字止於 08-19 21:02),唔好靠佢判斷生死
- [2026-08-20 05:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+45**(log累計 1610);重做隊剩 0;可做draft 239;producer 行緊

## 2026-08-20 05:38 — 三線平行複核 R1 國語線(04:53–05:38,43 分鐘)

**收工原因:冇貨做**(分區真空)。開波國語分區可做 124 首,收爐時 actionable 64 首全部係本班判過留 draft 嘅,未做過 = 0。**嚴禁 until-loop,冇等隊列**。

- **DB 數字**:verified 3812(開波 ledger 值)→ **3935**(收爐,含 R2 線同時段寫入);本班 verified 實數 **57 首**已用 SQL 核對。draft 544 / unavailable 727 / none 2962。
- **本班 125 個決定**:verified 57 / unusable 3(7587 聖誕串燒只有曲目卡、7846 界線 OCR+whisper 兩邊皆空、7822 希望在轉角 同上)/ delist 2(2180 見證訪談、6208 鍵盤手教學短片)/ 留 draft 63。
- **機械驗收**:十二批合共 58 個 apply entry,**58/58 pass、reject 0、langmismatch 0**;十二批 apply 後覆查全部零 race。
- **Producer**:keeper 全程生存(pid 8368),本班冇碰過 fetchLyrics。draft 池由 388 → 283。
- **WebSearch**:**4/4 用晒**。3 次有收穫(天韻《一件禮物》正字、《不為明天憂慮》「主的恩典天天夠用」、約書亞《即或不然的生命》「祂在我身旁我就不至動搖」),1 次零收穫(約書亞《出發》搵唔到,結果全部係同名流行曲)。
- **班尾 checkpoint**:`[stream]` 最後活動 17:13 UTC(healthcheck 三個固定 id,距今 4.5 鐘)→ 過關;approve.sh(HEAD 就係已批准 sha,零新 commit);backend-restart.sh 過 health check;`launchctl list | grep hymnapp` = **7** ✅;live 抽驗 5 首全部吐到歌詞。**串流健康零警報。**

**本班做過邊啲 vein / 心得(俾下一班):**
1. **天韻合唱團(主力,52 首舊存貨打出 43 首 verified)** —— 上一班判「舊碟靜態封面版差唔多全部死症」嘅結論**只啱一半**。靜態封面片個 draft 其實係 whisper 轉錄;whisper 質素兩極,**判準係「錯得有冇規律」**:全部同音錯(彈珠→但主、世家→十架、乘送野和荒→稱頌耶和華、葉和花→耶和華)就逐字改得返;整句意思跑晒(「把眼睛悄悄遠離 呼氣慢慢下樓梯」)就唔好夾硬。
2. **⭐ 天韻分區排序鍵**:標題有「【…】…Official MV」= 天韻影視中心近年重製、幾乎一定有燒死字幕,兩批共 15 首打出 11 首(73%);冇方括號嘅舊碟片約 10%。**下一班開波直接照呢個排。**
3. **⭐ 同名歌互補(三種用法,今班中咗 6 次)**:(a) 兩條片兩份獨立 ASR,分歧點就係要查嘅字(7760↔7740 零 WebSearch 救兩首);(b) 一條有 OCR 一條冇(8005 救 7717);(c) **同名歌已經 verified → 直接做底本**(7611→8015、8023→7764、7712→7772、1626→8631、8320→8262、6199→6385)。第三種要寫個 script:歌名 normalize 對碰,70 首 draft 撈到 17 個底本。
4. **陷阱**:實景 MV 嘅 OCR 會咬街景招牌落 draft(8092「台北101/香雞/優良協力廠」、7617「SAMDO/BRIGHT/HOLD OFF」),一律當浮水印剷。
5. **判死門檻(本班定)**:OCR 仲見到畫面文字證據(就算係「伊魡佟墳」呢類鬼字)→ 留 draft,換 OCR 引擎有機會救;**OCR 只有浮水印 + whisper 零內容 → 先判 unusable**。

**🔴 兩條要 Eric 確認嘅新政策(ledger 收爐附註有詳細版):**
- **「解唔到嗰句就剷,但唔因為一句解唔到就成首唔出」** —— 本班用咗 6 次。方向同「唔准屈歌詞」一致(寧願缺一句唔好爆一句假),但同以前「有缺口就留 draft」嘅慣例唔同。
- **英文歌版權分界線** —— 公有領域聖詩(O Holy Night 1847 / It Came Upon the Midnight Clear 1849)中英照出;天韻自家作品(Celebrate the Season,詞 Doris Brougham)照出;第三方在版權原作(Carman / Phil Driscoll & Lari Goss / Jester Hairston / John Telfer)**英文正文唔出**,有自家中譯就淨出中文、冇就標「暫緩-版權」留 draft。按呢條線我把 **7982 Mary's Boy Child 由中英對照回頭改做淨中文**。

**⚠️ 異常/待辦**:台語歌 `lang` 欄寫住「國語」累積 5 首已 verified(7986/8092/8262/8263/8080 留 draft)+ 3 首 draft(7669/7698/7750),metadata 要有人改,複核線改唔到。

**Fable 5 抽查名單(本班新 verify 隨機 8 首)**:7062 我主耶穌真好 / 8027【O Holy Night 男生三重唱】/ 7761 義人的福 / 7885 別讓地球再流淚 / 8262 詩篇二十三篇（台語）/ 6580 即或不然的生命 / 7726 主祢的手 / 8031【悔改的心 二重唱版】
- [2026-08-20 06:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+70**(log累計 1680);重做隊剩 0;可做draft 244;producer 冇行
- [2026-08-20 07:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+58**(log累計 1738);重做隊剩 0;可做draft 298;producer 行緊
- **心跳 2026-08-20 08:01** — verified 3981(64.0%)/ draft 361(06:37 keeper 讀可做 244)/ 重做隊剩 0 / producer 生存(pid 58307,06:37 起 OCR budget 120)/ ⚠️/tmp/hymn_fetchlyrics.log 嘅時間戳用 UTC(比本地慢 8 鐘),tail 見「00:01」其實係 08:01 實時寫緊,唔好誤判死咗
- [2026-08-20 08:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+31**(log累計 1769);重做隊剩 0;可做draft 324;producer 行緊
- [2026-08-20 09:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+31**(log累計 1800);重做隊剩 0;可做draft 182;producer 冇行

## 2026-08-20 09:40 — R1 國語線(lyrics-line-mandarin)08:52–09:40 HKT

**DB 前後:** curated verified **3991 → 4107(+116)**;全庫覆蓋率 **66.5%**(6180 首 curated)。
(+116 入面 84 首係本班做,其餘係 R2 粵語線前兩班寫入 DB、等我 restart 先出街嗰批。)

**本班數字:** 11 批、**162 個決定** —— verified **84**、unusable **20**、delist **45 個 id 提交(實際落架 36,另 9 個本身已落架)**、留 draft 58。audit 累計 **93 條入面 92 pass / 1 reject / 0 langmismatch**,11 批覆查全部零 race。WebSearch **4/4 用晒**(3 次有用、1 次落空)。收工原因:**冇貨做**(分區 123 首剩貨逐首讀過,九成係死症)。

**⚠️ 要 Eric 拍板 / 留意嘅三件事:**
1. **OCR 對台語否定詞會反轉句意。** 8088《輕輕叫著伊的名》PaddleOCR **五幀一致**讀出「上帝對咱永遠**會**變化」,但 whisper 兩次都聽到「**未**變化」—— 台語「袂」(bē)俾 OCR 一律讀成「會」。**單靠 OCR 出街會出反轉意思嘅歌詞**,今次留 draft 冇賭。建議台語歌一律要 whisper 覆核先出。
2. **shortOk 三次實測全部卡喺覆蓋率,唔係內容比例。** R2 前兩次 82%、我今次 7775 係 78%,三次都係「whisper 聽唔到尾」而唔係「whisper 聽到嘅多過你交嘅」。對「尾段長 outro」嘅短歌,85% 覆蓋門檻可能偏嚴。(7775 最後靠同曲異版 7713 補返全首歌詞出咗街,冇用到 override。)
3. **delist 咗兩類新嘢,如果 Eric 唔同意請叫我復原:** 天韻【讀經一 詩篇51】【讀經四 賽53】兩條**經文朗讀**片、新心《二十天求復興》**默想禱告集全套 21 條**、新心 2160《我們的禱告》(25 週年禱告文朗讀)。呢啲全部零唱段,但係屬靈內容,同「訪問/花絮」性質唔完全一樣。

**方法論收穫(建議寫入 SOP,下班照用):**
- ⭐⭐ **同曲異版互救 —— 本班單一最高產嘅招,一批 21 首入面 14 首靠佢。** 寫個 script 用「【】」/「＿」/括號/版本詞(字幕版/爵士版/男生重唱/不插電/動態歌詞版/韻流聲機)normalize 歌名再按 artist 分組,全庫掃出 25 組同曲異版,其中 14 組已經有一版 verified。**新 pattern:器樂版都可以救原曲** —— 8033「如果我能唱 小提琴演奏」打晒歌詞字幕,救返 OCR 全死嘅 7721 原曲版。**教訓:標題見到「演奏/伴奏」唔好即刻判純器樂。**
- ⭐ **逐幀時間戳 = 歌詞次序表。** dedupe 之後啲行冇次序,但 `lyrics_timeline.ocr` 每幀有 `t`,照 t 排就係唱嘅次序(8000 傳道書、7924 賽40 都靠呢招砌返)。第二遍副歌仲可以補返第一遍 OCR 漏咗嘅字(7998「生命**稍縱即逝**」)。
- ⭐ **唔好因為「OCR 出咗個怪字」就改。** 8060《願祢得高舉》我扣起咗兩批,以為「祢的慈愛**般**過諸天」係 OCR 錯;查完官方歌詞**正本真係「般過諸天」**,連罕見字「謳**咾**」都同 OCR 一模一樣。**清晰字幕上嘅怪字往往係真嘅。**
- **MV 片頭片尾嘅經文卡/旁白卡/AI 聲明卡一定要當註釋剷走**,唔剷會俾 audit 嘅經文括號擋板 reject(本班 7912/7863/7988/7999/8041/8094/8111/8599/8022 都有)。
- **中英雙語片只抄 CJK 行**,直接交上去一定觸發 langmismatch;本班 6 首雙語片(611/小羊/讚美之泉)全部零 langmismatch。
- **押韻/對仗可以做第三個實證來源**(7620「願我享受這**光榮**」由 空/中/動 押韻定案,推翻 whisper 嘅「寬容」)。
- **經文歌可以用和合本做第三來源**,但只限 whisper 逐句對得上嘅(7820 詩1 出咗街;7858 詩63、7898 詩121、7917 詩34 對唔實,照留 draft)。

**分區剩貨診斷(下班唔使重讀):** 剩 123 首國語 draft,四種死症 ——(1)天韻創作專輯**靜態封面**(OCR 只影到浮水印)、(2)**書法/藝術字幕**、(3)**純 whisper 冇 OCR**、(4)**medley 拆唔開**。已逐首讀過確認冇得做嘅 id:7796/7834/7661/7790/7755/7742/7743/7791/7729/7869/7663/7777/3879/3685/8638/7719/7850/7871/7788/7837/7838/7840/7829/7809/7793/7995/7631/8085/7961/8088/7858/7898/7917/7827/7666/7748/7671/7725/6026/8087/3723/3737/3531/5549/8096。**真正仲有機會嘅係 producer 新出爐嗰啲**(本班尾重 export 攞到 15 首新貨,出咗 10 首)。

**Fable 5 抽查名單(本班新 verify 隨機 8 首):**
| id | 歌名 | 底本來源 |
|---|---|---|
| 7620 | 在祢星中 In Your Galaxy | OCR+whisper+押韻 |
| 8000 | 生命的時序(傳3) | OCR 逐幀次序;⚠️「**爭戰**有時」係補字,OCR 冇影到 |
| 7820 | 溪邊的樹(詩1) | 純 whisper + 和合本對照(唯一一首用呢招出街) |
| 7930 | 祢的名何其美 | OCR「無**邊**寬廣」由孖生版 7609 + whisper 校做「無**限**」 |
| 7713 | 我要頌揚/宇宙歡唱 | OCR 清 |
| 8060 | 願祢得高舉 | OCR + WebSearch 核對(「般過諸天」「謳咾」保留原字) |
| 8604 | 榮耀中的呼喚(我心旋律) | OCR 清 |
| 7948 | 呂巧婕 寧靜的喜樂 | OCR 清(訪問字卡+3 張經文卡已剷) |

**串流健康:** 開波 grep `docs/SUPERVISION-LOG.md` 冇任何「🔴 串流健康警報」。班尾 [stream] 最後活動 2026-08-19T23:13Z(距 restart 2 個半鐘),restart 安全。
**Deploy:** approve 9a5db4c(查過只改 `frontend/hymn-app/src/screens/LibraryScreen.js`,非 backend 改動)→ backend-restart 過 health check → launchctl hymnapp job **7 個** ✅ → 抽 8604/7948/7713 curl live `/api/hymns` 全部吐到歌詞 ✅。
**紅線:** 零 git 操作、冇開 fetchLyrics、冇掂 Cloudflare/DNS、冇用 AskUserQuestion。keeper 全程生存(pid 8368)。
- [2026-08-20 10:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+29**(log累計 1829);重做隊剩 0;可做draft 194;producer 行緊
- **心跳 2026-08-20 11:01** — verified 4116(66.6%)/ draft 257(09:37 keeper 讀可做 182)/ 重做隊剩 0 / producer 生存(keeper pid 8368,09:37 起 OCR budget 120;fetchLyrics pid 88395 寫緊 log 到 11:00)/ 三個鐘 verified +135(3981→4116),draft 361→257、可做 244→182,複核線消化速度快過 P 線出貨,積壓收窄緊,正常
- [2026-08-20 11:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+28**(log累計 1857);重做隊剩 0;可做draft 220;producer 行緊
- [2026-08-20 12:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+28**(log累計 1885);重做隊剩 0;可做draft 247;producer 行緊

## 2026-08-20 13:32 — R2 粵語線(13:03–13:32)

**收工原因:冇貨做。** 分區 `lang='粵語'` 開波 54 首可做 draft(bi-freeze 全庫 249),做剩 16 首,而 16 首全部係本班親手判過要留低嘅,零新貨,冇 until-loop 等隊列。

**DB 數字:** 全庫 verified 4189(本班 +20)。累計決定 4167。

**本班成績:** 50 首讀 / **20 verified** / 7 unusable / **14 delist** / 16 留 draft / 0 langmismatch / 0 撞鎖 / 0 producer race / **WebSearch 0 次**(全部 `cantonhymnLookup.js` + DB whisper timeline)。

**出街 20 首:** 2985 啟示錄之歌、3801 你的愛、3803 與你一起、3026 眾聖徒獻忠誠、5676 凡人、208 每一天我需要祢、1721 聖殿站、2572 城不會下沉、1800 祢是葡萄樹我是枝子、3579 以基督為中心、1818 簡單的侍立、3023 最初的喜訊、3262 祈求主聖靈、2411 有時、6180 舊約目錄歌、181 凡事謝恩、162 保守我心、6151 主被人釘十架、1816 耶穌是我一生最愛、2342 祢使我抬起頭。

**Fable 5 抽查名單 8 首:** 2985 啟示錄之歌、1721 聖殿站、3579 以基督為中心、2411 有時、162 保守我心、6180 舊約目錄歌、2342 祢使我抬起頭、1818 簡單的侍立。

**🔴 要 Eric 拍板嘅一件事 —— `shortOk` 個 85% 覆蓋門檻結構上鎖死短兒歌:**
6156 祂是神、4733 親近天父爸爸、4362 ACM小天使之歌 三首都**唔係衰喺 1.6× 比例呢一關**(比例全部合格),係衰喺 whisper 覆蓋率 **84% / 82% / 82%**,三首齊齊爭幾個字位。原因好機械:短兒歌尾必定有 3–5 秒 outro 音樂或者靜音,whisper 最後一段就停喺嗰度,**1–3 分鐘嘅歌得 3 秒尾奏就已經蝕 2–4% 覆蓋**,永遠夠唔到 85%,歌越短越冇得救。呢批歌全部係真·天然短(cantonhymn 底本實錘全首得三四句),但機制上永遠出唔到街。
建議二揀一:(a) 片長 < 3 分鐘嘅覆蓋門檻降到 78–80%;(b) 改用「whisper 最後一段之後淨係得靜音/音樂」做判斷,唔用死比例。
**冇改 `auditLyricsBatch.js`**(SOP §3b 明令唔准),三首照留 draft 等拍板。

**方法心得(俾下班參考):**
1. **同名歌多粵譯版本** —— cantonhymn 一個歌名可以有 6 個唔同粵譯(2985 啟示錄之歌就係),**排第一嗰個唔一定啱**,一定要逐版對返自己 OCR/whisper 嘅字先揀。
2. **whisper 幻覺假 credit「李宗盛」係固定 pattern** —— 本班 6147 / 3803 / 1818 / 2411 / 3262 / 162 / 5036 / 5073 / 1816 九首都出現「詞曲:李宗盛」,係本地 whisper model 嘅固定幻覺,見到一律當冇,唔好當 credit 行剷。同類仲有「張偉俊 ×8」(6159)、「陳零九」(2411)、「張震嶽」(5676/1818)。
3. **中英對照字幕片 OCR 好靠得住** —— 181 凡事謝恩 whisper 全程 `[MUSIC]` 零幫助,但個片係中英雙行印刷字幕,OCR 極清,單源照做冇問題。呢類片同 lyric video 同級可靠。
4. **逐字動畫字幕片要 whisper 補句子結構** —— 1721 聖殿站 OCR 出嚟全部係散字冇句讀,靠 whisper 攞返句子邊界再用 OCR 定字,兩者缺一都做唔到。
5. **OCR 同 cantonhymn 打交時以自己底本為準** —— 3262 祈求主聖靈尾句底本係「誠實頌讚」但 SON Music 版 OCR 清楚係「來順服祢」,跟自己個片。
6. **悅雨音樂 Cafe live 系列(2750/2774/2781)確認死症** —— 現場片冇字幕淨係教會 watermark,OCR 永遠救唔返,已判 unusable。

**串流健康:** `backend/data/stream-health.log` 最新 2026-08-20 13:13 `ok=3 fail=0`,`docs/SUPERVISION-LOG.md` 冇新「🔴 串流健康警報」。

**⛔ 冇 approve、冇 backend restart(R2 冇權,SOP §1.4)。** 本班 20 首歌詞已寫入 DB,等 R1 國語線下一轉一齊放出街。**零 git 操作。**

## 2026-08-20 13:33 — R1 國語線(12:52–13:34 HKT)

**DB 前後:** verified 4107 → **4198**(+91,含 R2 同時段產出);draft 482;unavailable 829。
**我自己嘅數(149 個決定):** verified **52**、unusable **64**、delist **16**、留 draft **17**。
分區:開波 `lang='國語'` 可做 132 首 → 收工剩 17 首(全部係今班判過留 draft 嗰批,零新貨)。
**額度:** WebSearch **0 / 4** 次;全班**零 rate limit 警告**;冇撞 DB 鎖;12 批 apply 全部覆查**零 race**。
**Producer:** keeper 生存(pid 8368),班中途 40 分鐘補咗 13 首新國語 draft,所以做多咗兩批。

### 🛑 未做:approve + restart(91 首歌詞未出街)
- **唔係 stream 忙**:真用戶最後播放 2026-08-19T21:54Z(7.5 個鐘前);13:13 嗰三條 `[stream]`
  係 healthcheck 打固定 id 42/77/5431,`stream-health.log` 13:13 `ok=3 fail=0`,冇 🔴 警報。
- **係 SOP §5.2 嘅擋板**:approved sha `9a5db4c`,HEAD `677cf61`,中間有今日 12:20–12:21
  落嘅 BATCH6 六個 commit,C1/C2/C3 改緊 streaming hot path。
  `BATCH6-PLAN-20260820.md` §9.2 明寫「restart / OTA 係另一個決定,等 Eric 拍板逐批開」。
  backend 跑 working tree,restart = 連 BATCH6 一齊出,**冇得只出歌詞**。
- **要 Eric 一句話**:BATCH6 C1-C3 出 / 唔出。出 → 下轉 R1 一鍵 approve+restart;
  唔出 → 要 revert 咗三個 commit 先 restart 得。

### 方法論收穫(建議寫入 SOP)
1. ⭐**「OCR×whisper 二元組重疊率」偵測器**(`vein2.mjs` 邏輯):unique OCR 行有冇任何連續兩個中文字
   出現喺 whisper 全文。真歌詞片 >40%,**靜態封面片硬 0%**。一嘢掃出 19 首,肉眼覆核全中。
   假陽性兩種:英文歌、台語歌(whisper 用國語音轉)。**只准做候選名單,唔准自動判死。**
2. ⭐**靜態封面 vein 全貌**:天韻早期專輯每一輯都有一批「淨一張封面」嘅片,OCR 只影到專輯名/曲目表
   (「天额創作專輯之五」「伊煦熔憒」「穿越的塑晋」「另一種帱歌」…),款款唔同,**regex 永遠追唔切**。
   今班清咗 **56 首**,天韻國語積壓由 82 首打落去。
3. ⭐**推翻「medley 拆唔開 = 死症」**:lyric video 嘅 OCR 自帶時間戳,用 `【歌名】` 分段順時序寫就得。
   611/讚美之泉嘅現場串燒 **4 個決定買到 20 首歌嘅歌詞**,係全庫最抵做嘅貨,以後要優先揀。
4. ⭐**經文歌三方對齊**:OCR 碎到剩單字都救得返 —— OCR 散字 + whisper + 和合本經文互鎖。
   今班靠呢招出咗 8096(雅歌)、7995(賽25)、6745(雅歌)。
5. ⭐**唔可以靠 title 判語言**:611 好多歌 title 淨英文(Spirit of Hope / Jesus Is Alive /
   How Great is our God),片入面字幕係**全中文**,一開就骨。前面班次好可能因此 skip 咗。
6. ⭐**版權界線收窄講法**:R3 停線嘅真理由係「唔准重現**在版權英文歌詞正文**」,唔係「作品出身係英文」。
   中文譯詞字幕照做(否則同批出街嘅 How Great is our God / Worthy of It All 都要扣)。
   我批5 一度用錯界線扣起 7906/7911,批8 改返。
...
### 要 Eric 過目嘅判斷(三項)
1. **8088 輕輕叫著伊的名**:三個字**逆住 OCR 改**(會→袂、晦→毋),理由係語意 + whisper 佐證,
   台語漢字俾 PaddleOCR 讀錯。要嚴格死跟 OCR 就要拉返。
2. **611 中文原創 + 英譯字幕(3498/1887/2892/3737…)只寫中文入 DB**,跟返庫入面已 verified 嘅
   611 歌(1528/1538/1539)全部中文單語嘅慣例。要連英譯一齊入就要重做。
3. **7785 平安**:片頭 3 分半係歌、後半係「服事甘苦談」訪問。歌完整所以出咗街,
   但如果覺得半片訪問唔應該留喺庫就要另行落架。

### 仍然扣起(唔 apply 唔判死)
- **8368 離此不遠**(中英對照但 OCR 英文行唔齊)、**8093 透過信心的眼睛**(英文正文完整,在版權)。
- **7671 馬利亞之歌**(尊主頌 路1:38+46-55)、**7666 靠主誇口**(耶9:23-24):判咗 unusable 但
  兩首都係純經文歌,第日可以唔靠 OCR、直接用和合本人手補返。
- [2026-08-20 13:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+31**(log累計 1916);重做隊剩 0;可做draft 104;producer 行緊
- **心跳 2026-08-20 14:01** — verified 4188(68.1%)/ draft 178(可做 60)/ 重做隊剩 0 / producer 生存 / 重做隊已清零、draft 池 178 入面 118 首係 bi-frozen,真正可做得 60 首,keeper 12:02 嗰輪仲報「可做 231」同而家對唔上,OCR 產出速度似乎追唔上消化
- [2026-08-20 14:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+38**(log累計 1954);重做隊剩 0;可做draft 142;producer 行緊
- [2026-08-20 15:37] P線時報(keeper自動):過去1小時 OCR/whisper draft **+30**(log累計 1984);重做隊剩 0;可做draft 168;producer 行緊

## 2026-08-20 15:43 — ⛔ lyrics-daily-proofread 15:40 班煞停(零決定、零 DB 寫入、零 restart)

**煞停原因兩個,兩個都係結構性,唔係當場出錯:**

### 1. 呢條線冇分區,一做就違反 `ops/lyrics/REVIEW-LINE-SOP.md` §1.1
SOP 明文:三線靠 `lang` 欄硬分區先至唔撞單,「**做唔屬於你分區嘅歌 = 直接違規**」。
`lyrics-daily-proofread` 個 SKILL.md 係 2026-08-01 寫,冇分區概念,export 一嘢攞晒全部 234 首。
今日開波實況(`bi-freeze --filter` 之後可做 **170** 首):

| lang | 可做 draft | 邊條線嘅分區 | 狀態 |
|---|---|---|---|
| 國語 | 59 | **R1 `lyrics-line-mandarin`** | ✅ enabled,下一班 **16:52 HKT**(69 分鐘後) |
| 粵語 | 39 | **R2 `lyrics-line-cantonese`** | ✅ enabled,下一班 **17:03 HKT** |
| 英文 | 72 | ~~R3~~ | 🛑 2026-08-19 Eric 拍板停線,版權暫緩,SOP 明文「**draft 保留唔郁**」 |

三格冇一格輪到我:國語粵語做落去就係搶 R1/R2 一個鐘後嗰兩班嘅貨(白燒兩份額度 + `--apply` 撞單,
已知 producer race 實錄 76 中 1);英文做落去就係直接踩版權暫緩令。
**同一個原因,呢條線 2026-08-19 已經煞停過兩次**(見本 log 5834 / 5895 行)。今次係第三次。

### 2. 就算做完都出唔到街 —— deploy gate 卡住,而且我冇 restart 權
- `~/.hymn-deploy/approved.json` backend sha = `9a5db4c`,HEAD = `677cf61`,中間有今日 12:20–12:21
  落嘅 BATCH6 六個 commit(C1/C2/C3 改緊 streaming hot path)。restart = 連 BATCH6 一齊出街。
- SOP §1.4:**restart 權單一 owner 係 R1**,其他線一律唔准 approve / restart。
- R1 13:33 班尾已經因為同一個 gate 停手,**91 首歌詞而家仲喺 DB 未出街**,等 Eric 拍板 BATCH6 出唔出。
  我今日就算再 verify 幾多首,一樣係堆埋喺同一個等候區。

**所以:冇 export 之後嘅任何動作 —— 零校對、零 audit、零 `--apply`、零 restart、零 git 操作。**
...
### 要 Eric 拍板嘅兩件事(接住 R1 13:33 嗰張單,冇新嘢)
1. **BATCH6 C1-C3 出唔出?** 出 → R1 下一班一鍵 approve+restart,91 首歌詞即刻出街;
   唔出 → 要 revert 三個 commit 先 restart 得。
2. **`lyrics-daily-proofread` 應唔應該停用?** 三線 SOP 上場之後呢條線結構上冇貨可做,
   每班淨係燒一次 export + align(align --all 行足 2 分鐘)換一份煞停報告,今日已經係第三次。
   建議直接 disable,留返 R1/R2 兩條線 + P 線就夠。

## 2026-08-20 15:47–15:52 — ⏸ 歌詞 pipeline 全面暫停(Eric 要開 Xcode 模擬器做 native build)

**原因:** Eric 即將開 Xcode 模擬器做 native build + 實測,OCR pipeline(PaddleOCR / whisper / ffmpeg)同佢爭 CPU / 記憶體。

### 停之前狀態(方便之後恢復)
| 指標 | 數 |
|---|---|
| **verified** | **4,188 / 6,150 = 68.1%** |
| draft | 236(可做 171) |
| none(可攻池) | 964,其中 OCR 池(cc:miss)775 |
| unavailable | 762 |
| **重做隊** | **0 —— 573 首全部做完** ✅ |

### 停法(安全次序)
1. `touch /tmp/lyrics-sprint-stop` —— keeper 見到就唔會再開新 run。
2. **等手頭嗰首做完先殺**:等咗 5 分鐘 whisper/paddle 收工。實錄:id 114 喺 07:49:24 UTC 寫低「✓ OCR 有效草稿(累計 +119)」之後先落 SIGTERM,**冇殺到一半**。
3. `SIGTERM`(唔用 `-9`)fetchLyrics → whisper/paddle → keeper → caffeinate。
   - 補充:就算真係中途殺都**唔會爛 DB** —— 每首寫入行 `acquireDbLock` + `saveDb()` 係 temp+rename 原子寫,最壞情況只係嗰首白做,留返池入面下次再嚟。
4. 核實:`pgrep` 零 OCR/producer process。

### 停咗嘅嘢
| 項 | 狀態 |
|---|---|
| producer-keeper + fetchLyrics + paddleframe + whisper-cli | ⏸ 已停 |
| caffeinate ×2 | ⏸ 已停(部機恢復正常省電行為) |
| `lyrics-line-mandarin`(R1) | ⏸ disabled |
| `lyrics-line-cantonese`(R2) | ⏸ disabled |
| `lyrics-daily-proofread` | ⏸ disabled |
| `lyrics-progress-heartbeat` | ⏸ disabled |
| `lyrics-line-english`(R3) | 早前已 disabled(版權,見 8/19) |

### **冇停**(刻意保留)
- **backend server** —— App 照用得,7 個 job 齊。Eric 做模擬器實測需要佢。
- **`com.hymnstream.healthcheck`**(每 6 鐘 3 個 curl)—— 負擔可以忽略,而且佢做緊實測嗰陣正好幫手監察串流健康。

### 恢復做法(等 Eric 講聲)
```bash
rm -f /tmp/lyrics-sprint-stop
cd /Users/macbookpro/.openclaw/workspace/hymn-app
nohup bash ops/lyrics/producer-keeper.sh >/dev/null 2>&1 & disown
nohup caffeinate -dims >/dev/null 2>&1 & disown
```
再用 scheduled-tasks MCP 開返 `lyrics-line-mandarin` / `lyrics-line-cantonese` /
...
- **心跳 2026-08-22 08:01** — verified 4970(81.6%)/ draft 194(可做 128)/ 重做隊剩 17 / producer 生存 / ⚠ OCR 池空轉:07:26 同 07:31 兩轉都話「冇更多 cc:miss 嘅歌等 OCR,淨低嗰啲俾 ledger / --skip-orgs 剔走」,keeper 已自動排下一轉轉 CC 補倉——即係 128 首可做 draft 唔係額度問題係池被濾網剔清

## 2026-08-22 08:15 — 53 首「真死連結」查實**唔係死連結**,已 reset(唔使搵替代、唔使落架)

**Eric 指示:** 逐首搵返替代 YouTube 連結,搵唔返先落架。
**執行結果:唔使做 —— 前提唔成立。**

### 兩個實測證據
1. **53 首全部 `fail_streak = 0`** —— 每晚 `checkDeadLinks` 職責就係驗連結生死,佢話呢批**冇死**。
   ⚠️ 概念要分清:`dl:dead` 係**我哋自己**「落載失敗 3 次」嘅標籤,**唔等於條 link 死咗**。兩件事一直用同一個「dead」字,容易誤導 —— 呢次差啲就因此剷咗 53 首好歌。
2. **隨機抽 8 首用 nightly yt-dlp 真落載:8/8 全部成功**(9.1M–21M),包括 6:44 / 7:13 嗰啲長片。

### 點解會喺 8/20–21 判死
嗰兩日**正正係模擬器搶 CPU 嗰段時間**(Xcode build + 播放實測),OCR pipeline 同佢爭資源,落載 300s timeout 好易撞到;加上零散 403(未夠 5 次連續,觸發唔到全域封鎖掣)照樣逐首記賬,夠三次就判死。

### 處置
照前兩批做法 reset:**DB `dl:dead`→`cc:miss` + 清 ledger 53 條**。`dl:dead` 而家**歸零**,OCR 池 609 → **662**。名單:`backend/data/dl-dead-reset-20260822.json`。

**Eric 嗰個「搵替代 / 落架」方案冇作廢,只係押後**:真係壞嘅片會自然再失敗三次、再攞返 `dl:dead` 標籤 —— 到時嗰批先至係真候選,先值得花 WebSearch 額度搵替代片源。而家花就係浪費。

### 累計:三批 dl:dead reset
| 批 | 首數 | 命中(落到片) |
|---|---|---|
| 8/18–19(403 風暴) | 685 | ~69% |
| 8/16–17(舊 yt-dlp 年代) | 181 | **100%** |
| 8/20–21(模擬器爭 CPU) | 53 | 抽驗 8/8 |
| **合計** | **919 首** | 全部本來會永久冇咗 |

- [2026-08-22 09:00] P線時報(keeper自動):過去1小時 OCR/whisper draft **+18**(log累計 2926);重做隊剩 0;可做draft 150;producer 行緊

## 2026-08-22 09:55 — 每日自動歌詞校對 routine(lyrics-daily-proofread,09:43–09:55)

**現況:** export 得 **215 首 draft**(粵 49 / 國 82 / 英 82 / 兒童 2);`alignLyrics --all` 行足 ~3 分鐘,213 首有對齊數據(2 首冇 timeline)。收工時 DB:verified **5011** / draft **595** / unavailable **350** / none 2215。live `/api/hymns` 6094 首、有歌詞 **4978**(⚠️同 DB verified 差 **33 首**,見下面「冇 restart」)。

**決定分佈(215 首全讀完):** verified **0** / unusable **12** / 留 draft **203** / demote 0 / reject 0。
audit **12/12 一次過骨**(rejects 0、langmismatch 0),apply 全中,逐條覆查 `lyrics_status='unavailable'` 已落實、`lyrics_draft` 原文全部保留。

**verified = 0(第四日一樣):** 呢個 routine 出唔到 verified —— 系統版權紀律禁止複製/重寫第三方歌詞全文,而 `--apply` 要 `lyrics` 欄填足全文。自動班嘅實際產出只能係**分流**。已經連續四日,建議 Eric 拍板:改 routine 定位做「純分流班」,定係搵第二個唔受同一限制嘅執行者。

### 今轉判死嘅 12 首

判死規則(**今轉修訂咗昨日嘅版本,見下面「兩條規則要改」**):

| id | 歌名 | 判死理由 | ws / uniq / inv |
|---|---|---|---|
| 276 | 天韻【QT音樂】敬拜篇 Quiet Time Music | 底本**全部係經文**(約1:14/詩59:16/詩25:14)零歌詞行;QT 器樂 | 573 / 2 / 18 |
| 3720 | My Life Is In You/ You Are Good/ Goodness Of God | **28:51 三首連唱全場敬拜**,唔係單曲,冇法拆 | 278 / 2 / 282 |
| 3367 | Hosanna (粵語譯版**1-7合併版**) | 7 個唔同粵譯版合併,冇法拆;OCR 全亂碼(「也/必/我」碎片) | 18 / — / 28 |
| 5645 | 主のみてに導かれて (SOP Japanese Album 01) | 日文標咗「國語」(同 8-21 判死嘅 6350 同一類) | 10 / 1 / 36 |
| 5681 | リバー・オブ・ジョイ (SOP Japanese Album 01) | 日文標咗「國語」 | 33 / — / 48 |
| 5916 | 讚美之泉**弦樂四重奏** My Sweet Jesus | 純器樂:whisper 全 `[MUSIC]`,OCR 59 行全亂碼 | 8 / 1 / 59 |
| 5924 | 讚美之泉**弦樂四重奏** The Path of Grace | 純器樂:whisper 全「詞曲:李宗盛」幻覺,零 OCR | 9 / 4 / 1 |
| 5983 | 讚美之泉**安靜系列** (1) Come Away With Me | 純器樂:27 段全部「詞曲:李宗盛」幻覺,零 OCR | 27 / 3 / 1 |
| 3036 | 主寶血 (O the Blood 廣東話版) | 零 OCR + whisper 100% 幻覺 loop;冇粵語 verified 兄弟片 | 16 / 1 / 1 |
| 3038 | 讚美救主 (Praise Him! 粵語版) | 零 OCR + whisper misheard 亂碼 104 字;零兄弟片 | 13 / — / 1 |
| 3045 | 我要順服 (讚美之泉 粵語版) | 零 OCR + whisper 全「詞曲:李宗盛」幻覺;國語兄弟片救唔到粵譯詞 | 15 / 4 / 1 |
| 6052 | 角聲使團《因祢而起》[Bass by Ah Sai] | 零 OCR + whisper 100%「詩歌歌曲,詩歌歌曲。」幻覺 loop;零兄弟片 | 8 / 1 / 1 |

**同系列已判死佐證(唔係孤例):** 弦樂四重奏系列另外 **11 首**、安靜系列另外 **9 首**、ACM樂隊系列 02 已經係 `unavailable`;5916/5924/5983 係嗰兩個系列最後嘅漏網。

### ⚠️ 兩條判死規則今轉要改(重要,寫低俾後人)

**(1) 「whisper segs < 20 先准判死」呢條門檻唔啱用,要改用「whisper *unique* segment 數」。**
今轉實測到幻覺 loop 可以有好高 segment 數:`2768` ws=**88** uniq=1、`3720` ws=**278** uniq=2、`276` ws=**573** uniq=2 —— 全部係同一句重複幾百次。反過嚟 `645`(Matt Redman)ws=46 有 6 個 `[MUSIC]` 但**內容係完整英文轉錄**,係好底本。
→ 新判準:**whisper 死 = unique segment ≤ 4**(唔理 ws 幾多);`[MUSIC]` 標記本身**唔算**死症指紋。
→ 已知幻覺指紋(今轉新增兩條):「詩歌歌曲,詩歌歌曲。」/「詩歌歌詞的錄音,粵語或國語敬拜讚美詩歌。」loop、「詞曲:李宗盛」loop、`[MUSIC]`×N、`[singing in Cantonese]`×N、`[FOREIGN]`×N。

**(2) 昨日嘅判準 (c)「台語內容標咗國語 → unusable」係錯,今轉推翻。**
全庫查過:**13 首台語歌已經係 `verified`**(8249/8262/8263/8264/8266/8268/8270 + 讚美之泉台語系列 5246/5250/5251/5258/5284/5285…),全部掛 `lang='國語'` —— 即係台語有生產線,判死會**毀咗真嘅產能**。
→ `8269`(稱頌祢聖名（台語）)今轉**冇判死**,留 draft。日文(5645/5681)照判死,因為日文專輯零 verified、亦冇日文校對線 —— 但呢個係**政策題,建議 Eric 拍板**:日文/台語要唔要開自己嘅 lang 桶,定係一律判死。

### ⚠️ 「同名 verified 兄弟片」檢查救返 5 首(今轉最大收穫)

判死前逐首查「全庫有冇同一首歌、已經 verified 嘅第二條片」,**5 首本來會枉死嘅由判死名單剔返出嚟**:

| 差啲判死 | 救返嘅兄弟片 |
|---|---|
| `5448` ACM樂隊系列 01 祝福你天天讚美(OCR 淨係和弦圖) | `5400` 【祝福你天天讚美】ACM Official Lyric Video ✅verified |
| `6044` 角聲使團《祢恩手永在》(零 OCR + 幻覺) | `6006` 角聲使團《祢恩手永在》Official Lyric Video ✅verified |
| `3384` 與祢一起 廣東話版(零 OCR + 幻覺) | `3803` 與你一起(永遠敬拜 廣東話版)✅verified |
| `3627` 同心圓《真愛》(OCR 淨係 branding) | `3075` 同心圓 真愛(苦路十四站)Official MV ✅verified |
| `3634` 同心圓《To Love》(幻覺 loop) | `3607` 同心圓《To Love》TWS「LOVE」Live ✅verified |

→ **呢個 check 由今日起列入判死必經步驟**。留意 `5420` ACM樂隊系列 **03** 都係 verified,即係「Band Cover」呢個系列**唔可以一竹篙打一船人**當純器樂。

### 另一個重要觀察:**matchRate=0% 大部分係 whisper 死,唔係 OCR 差**

今轉 38 首 `mr=0%` 入面,好多其實 OCR inventory 好厚(`6861` inv=294、`3720` inv=282、`6803` inv=275、`6650` inv=194、`5448` inv=209、`7532` inv=145),對唔到純粹係因為 whisper 軌係幻覺 loop。
→ **呢批係人手線最抵做嘅貨**(OCR 底本完好,唔使等 whisper),唔好見 low-confidence 就當死症。點名:`6650` `6861` `6803` `7532` `7178` `7471` `6593` `6607` `6659` `6622` `6611` `3772` `2896` `7980` `5277`(大部分係約書亞樂團官方歌詞 MV)。

### 特登**冇**判死嘅邊緣個案(留低理由,唔好下輪再重複讀)
- `8269` 稱頌祢聖名（台語）—— 見上面規則 (2),台語有 13 首 verified 產線。
- `5448` / `6044` / `3384` / `3627` / `3634` —— 有 verified 兄弟片,見上表。
- `645` Matt Redman The Praise Is Yours —— `[MUSIC]` 只係間奏標記,whisper 係完整英文全曲,好底本。
- `3354` 長存萬世 / `3013` 世界之光 / `3377` 滿有能力 / `3044` 擁戴耶穌為王 / `3046` 我確信 / `135` 恩典太美麗 / `2901` 歌頌主聖名 —— CantonHymn 粵譯 cover,零 OCR 但 whisper 有**真內容**(只係聽錯字),R2 線可以用 `cantonhymnLookup.js` 核對救。
- `8307` 阿們(小羊詩歌)—— ws=0 冇 whisper、OCR 得 2 句(57 字),太薄但唔係亂碼,按規則留 draft。
- `2751` / `2768`(悅雨音樂盒 2016 分享會)—— whisper 係幻覺,但 OCR 有真歌詞行,太薄留 draft。
- `6161` 兒童詩歌307 / `6167` 你是牧人極愛我 —— 天然短兒歌 + 注音符號雜訊,OCR 內容真,留 draft。
- `3931` Fix You (Live Cover) // Milk&Honey —— OCR 全亂碼但 whisper ws=32 有完整英文。⚠️**Coldplay《Fix You》世俗歌 cover,係咪應該留喺詩歌庫,交監督線判**(唔屬校對班權限)。

### 🔎 交監督線嘅兩張清單(唔屬呢個 routine 權限)
1. **建議 delist 5 首:** `276`(QT 器樂+經文)、`3720`(28:51 全場敬拜合輯)、`5916`+`5924`+`5983`(讚美之泉純器樂系列尾巴 —— 同 INSTRUMENTAL-CATEGORY-PLAN §8「伴奏唔收」同一類)。另加一個**判斷題**:`3931` Fix You(Coldplay 世俗歌)。
2. **31 首「宣傳短片 / 專輯試聽 / 預告」仲喺 draft/none,冇一首入到今日 export**,即係佢哋唔會被自動班掃到,要獨立一 pass 清:`4993 5762 6058 6112 6254 6293 6329 6417 6423 6462 6522 6523 6525 6528 6532 6548 6549 6550 6551 7155 8538`(draft)+ `1704 2289 2491 3074 5778 6418 6422 6465 6552 8560`(none)。⚠️注意 `6355`(2015 兒童專輯6 宣傳短片)竟然係 **verified**,清嗰陣順手覆查。
3. **KALA/伴奏版仍然收緊:** `6931`、`7071`(約書亞樂團官方 KALA 版)仲喺 draft。按 INSTRUMENTAL-CATEGORY-PLAN §8「伴奏唔收」應該係 delist 對象唔係校對對象,留返 Phase 1 掃描處理(同 8-21 一樣冇動)。

### 其他
- **WebSearch 用咗 0/30 次;cantonhymnLookup 冇用**(今轉冇一首行到「真歌 + low-confidence 要核對」呢一步,因為出唔到 verified)。
- **yt-dlp / YouTube 零接觸**,全程本地 + 一次 localhost curl。
- **⛔ 冇做 restart(刻意,同 8-21 一樣):** 今轉零 verified,自己冇嘢要出街。live 4978 / DB verified 5011 **差 33 首**,嗰批係 R1/R2 線 8-21 夜晚之後做嘅,**restart 權喺人手線**,自動班唔搶行(亦避免撞正 Eric 真機 QA)。`lyrics_status='unavailable'` 純內部隊列狀態,唔 restart 對用戶零影響。

**Fable 5 抽查名單(今轉 verified=0,改抽判死個案):** `276`(經文非歌詞)、`3720`(28:51 合輯)、`3367`(1-7合併版)、`5681`(日文)、`5983`(安靜系列器樂)、`3045`(李宗盛幻覺)、`6052`(詩歌歌曲 loop)、`3038`(零 OCR 亂碼)。另**請重點覆核「兩條規則要改」同「兄弟片救返 5 首」兩節**,嗰兩節推翻咗 8-21 嘅判準。
- [2026-08-22 10:00] P線時報(keeper自動):過去1小時 OCR/whisper draft **+60**(log累計 2986);重做隊剩 0;可做draft 158;producer 行緊
- [2026-08-22 11:00] P線時報(keeper自動):過去1小時 OCR/whisper draft **+59**(log累計 3045);重做隊剩 0;可做draft 215;producer 行緊
- **心跳 2026-08-22 11:01** — verified 5001(82.1%)/ draft 279(可做 199)/ 重做隊剩 0 / producer 生存 / 重做隊已清零、10:45起開緊OCR batch(budget 120,兩條PaddleOCR並行),正常
- [2026-08-22 12:00] P線時報(keeper自動):過去1小時 OCR/whisper draft **+62**(log累計 3107);重做隊剩 0;可做draft 273;producer 行緊
- [2026-08-22 13:00] P線時報(keeper自動):過去1小時 OCR/whisper draft **+65**(log累計 3172);重做隊剩 0;可做draft 334;producer 行緊
- [2026-08-22 14:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+79**(log累計 3251);重做隊剩 0;可做draft 333;producer 行緊
- **心跳 2026-08-22 14:01** — verified 5073(83.3%)/ draft 407(可做 313;bi-frozen 335)/ 重做隊剩 0 / producer 生存(keeper pid 28362)/ 早幾轉「OCR 池抽乾」已解:12:45 keeper 報池 423、開緊 OCR batch(budget 120),draft 池由 11:01 嘅 279 升到 407,verified 三個鐘 +72,正常
- [2026-08-22 15:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+58**(log累計 3309);重做隊剩 0;可做draft 388;producer 行緊

## 2026-08-22 15:50 — 複核線由 2 條加到 4 條(Eric 拍板)

**點解要加:** producer 出貨快過複核 —— draft 隊列由 263 升到 **400**,再升就會撞到 keeper 個 400 上限令 producer 自己唞低,即係樽頸由 OCR 轉咗去複核。

### 分區方法:喺原有 `lang` 之上再用 `id % 2` 拆一刀

| 線 | Task | 分區 | 開波存貨 | cron | Restart |
|---|---|---|---|---|---|
| R1 | `lyrics-line-mandarin` | 國語 **單數 id** | 99 | `45 12,16,20,0,4,8` | ✅ 唯一 |
| **R1b(新)** | `lyrics-line-mandarin-b` | 國語 **雙數 id** | 113 | `45 14,18,22,2,6,10` | ❌ |
| R2 | `lyrics-line-cantonese` | 粵語 **單數 id** | 34 | `0 13,17,21,1,5,9` | ❌ |
| **R2b(新)** | `lyrics-line-cantonese-b` | 粵語 **雙數 id** | 36 | `0 15,19,23,3,7,11` | ❌ |

**點解用單/雙數,唔用 id 範圍或者搶佔式 queue(Eric 要求 #2):**
- **零重疊、零協調**:純數學保證兩條線攞唔到同一首,唔使 lease、唔使鎖、唔使問隔籬線做緊乜。
- **自動平衡**:實測國語 99/113、粵語 34/36,兩邊份量差唔多。
- id 範圍會隨新歌入庫失衡;搶佔式 queue 要改 `reviewLyrics.js` 加 lease 欄,風險大好多、要改核心 script。

**DB lock:** apply 攞鎖只係一瞬(唔係揸住做慢嘢),而且四條線 cron **各自錯開至少 1 個鐘**(每日 24 個時段輪住開),同時 apply 機會極低。撞到照原規矩等,5 分鐘唔得就跳下一批。

### 質素冇放水(Eric 要求 #1)
兩條新線用**完全一樣**嘅 SOP:只准 apply `-passed.json`、中英混合全面扣起、`shortOk` 要 whisper 實證、唔准按 artist/org 自動判 unusable、版權紅線、apply 後覆查 race。R1 仍然係**唯一** restart owner,其餘三條線嘅成果全部靠佢放出街。

### 預計消化速度(Eric 要求 #3)
兩條線嘅時候每 4 個鐘一轉;而家四條線,**每日複核觸發次數由 12 次 → 24 次**,理論吞吐翻倍。實際受每班 200 決定上限同分區存貨限制,預計 draft 隊列由「持續上升」轉為**今晚開始回落**。⚠️ 呢個係預測唔係實測,下轉之後睇返 ledger 先算數。

- [2026-08-22 16:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+31**(log累計 3340);重做隊剩 0;可做draft 383;producer 行緊

## 2026-08-22 15:43–16:05 — 每日自動校對 routine(第五轉)

**現況:** export 480 首 draft(粵85/國266/英127/兒2),`alignLyrics --all` 出 5522 行對齊數據。

### 今轉數字
| 項 | 數 |
|---|---|
| verified | **30**(全部靠「同名 verified 兄弟片」救返,零手打歌詞) |
| unusable(踢出隊列) | **10** |
| 留 draft | 440 |
| reject | 0(`auditLyricsBatch` 40/40 過,langmismatch 0) |
| DB verified | 5083 → **5113** |
| DB draft | 877 → 838 |
| DB unavailable | 351 → 361 |
| WebSearch | **0/30** |
| cantonhymnLookup | 3 次(全部核唔到 / 撞片語唔算核到) |
| yt-dlp / YouTube | 零接觸 |

### 突破:「重覆上載兄弟片」可以做成一條自動化 vein(30 首)
之前四轉都係「自動 routine 出唔到 verified」。今轉行通咗一條唔使人手打字、亦唔掂第三方歌詞網嘅路:

1. 全庫掃 **同 artist + 同名(剝走 Official/MV/Lyrics/Visualiser/Actions Video 等格式詞)+ duration 差 ≤3 秒** 嘅 verified 兄弟片。
2. 攞 draft 自己嘅 **OCR 行 + whisper segment** 去對兄弟片嘅 verified 歌詞,計命中率做實證(唔係淨靠 metadata)。
3. 命中率 ≥30%,或者**絕對命中行數 ≥7**(OCR 有大量水印/credit 雜行拉低分母,所以絕對數比百分比可靠),先算核到。

⚠️**三個實測陷阱**(下轉要記住):
- **normalize 唔可以剝 noise word**。第一版剝埋 `worship/song/the/kids` 之類,`【靠主得勝】兒童敬拜讚美(7)` 同 `【耶和華沙龍】兒童敬拜讚美(5)` 會 normalize 到一模一樣 —— 1870 個假配對。只准剝**影片格式詞**(Official / MV / Lyrics / Visualiser / Actions Video / Dance Video / Audio / HD)。
- **containment ratio 卡太緊會漏返最緊要嗰啲**。`6292` 同 `5838` 都係「最美的禮物 敬拜讚美(14)」,但一個標題長一個短,ratio 0.4 就被 0.55 門檻篩走;`2688 完了/音樂分享會` 對 `2585 完了/Lyrics` 一樣。改成「放寬 ratio + 強制內容對證」先兩樣都攞得返。
- **百分比會冤枉好 match**。`7212 讓家偉大 English Version` OCR 只得 24%,但絕對命中 **19 行** —— 分母 78 行入面大半係水印。用絕對數救返 7 首。

### 今轉 verified 30 首(全部 draft→verified,來源=自家 DB 已 verified 兄弟片)
`4555←4556` `4802←4447` `4540←4571` `4801←4444` `4803←4786` `4799←4443` `4784←4429` `4797←4441` `5742←5730` `4545←4571` `7805←7944` `4470←4487` `7190←6757` `7892←7877` `4796←4436` `3140←3979` `6607←6598` `7471←7369` `7212←7183` `6622←6606` `6593←6592` `4433←4445` `6611←6605` `6615←6604` `6292←5838` `2688←2585` `1289←8480` `6020←6016` `7888←7880` `7893←7878`

其中四首係**內容對證唔到、純靠 metadata 鎖死**嘅,請重點覆核:
- `2688`(完了 / 余幸蓓 / 音樂分享會)← `2585`(完了 / 余幸蓓 / Lyrics):OCR 淨得教會 branding、whisper 7 段 2 unique(死),但同歌手同歌名 duration 2:57 vs 2:56。
- `6292`(最美的禮物 敬拜 MV)← `5838`(最美的禮物 官方歌詞版 MV):同係「敬拜讚美(14)」系列,4:21 vs 4:22。
- `1289`(十字路口,盛曉玫)← `8480`(十字路口 Crossroads,泥土音樂,同專輯《信心》):**1289 duration 係 null**,對唔到秒數,靠 title+album 全等。
- `6020`(最好的福氣 Saxophone 版)← `6016`:薩克斯風演奏版,但片入面有打歌詞,OCR 十行都係正歌;4:12 vs 4:11。

### unusable 10 首(底本判死,draft 原文保留)
| id | 歌名 | 判死理由 |
|---|---|---|
| 739 | 《Amazing Piano 2》基恩敬拜鋼琴靈修音樂 | 57:58 純鋼琴合輯,OCR 淨係得曲序水印 |
| 3230 | 【不只是浪漫】鋼琴版 | 純器樂,whisper 得 credit loop |
| 3989 | 與你同走過－大提琴演奏 | 純器樂,OCR 淨係新心 branding |
| 5699 | 【能不能】鋼琴演奏系列(3) | 純器樂,OCR 淨係專輯 branding |
| 5175 | 《安靜敬拜時分》無插電【三首】EP24 | 9:47 medley 拆唔開,whisper 21 段 3 unique |
| 3865 | 苦難中有祢 Fellowship in Suffering | whisper 全 `[MUSIC]` 佔位符,零 OCR,全庫冇兄弟片 |
| 7747 | 上帝超過風雨和大海(台) | OCR 5 行全亂碼異體字,whisper 11 段 2 unique |
| 7724 | 上帝甲咱惜命命(台) | OCR 13 行全亂碼異體字,whisper 47 段 4 unique |
| 6161 | 【兒童詩歌307】任何遭遇不要驚怕 | OCR 得 3 行,whisper 係「詩歌歌曲,詩歌歌曲。」幻覺,cantonhymn index 冇 |
| 7819 | 不會太遲(天韻) | OCR 淨係「天韻創作專輯之五」水印,whisper 16 段 4 unique |

⚠️ `7747`/`7724` 係台語,但判死理由係**底本亂碼**唔係語言 —— 冇違反 8-22「台語唔可以判死」個修訂。

### 等 Eric / 監督線處理(自動班冇權做)
1. **建議 delist 1 首非歌內容:** `7592`《葉薇心的金曲作詞課》｜葉姐教你寫歌詞 搶先看 —— OCR 實錘係作詞課程講稿,唔係歌。按 `reviewLyrics.js` 註解,非歌內容行 delist 流程唔用 unusable,所以今轉冇動佢,**佢下轉會再出現喺 export**。
2. **建議 `739` 一併 delist:** 已標 unavailable,但佢本身係 57:58 全碟合輯,按 INSTRUMENTAL-CATEGORY-PLAN §8「上限 10 分鐘」都唔應該留喺庫。
3. **3 首日語版 lang 標錯做「國語」,但唔好判死:** `5666`(天が開き Open Heaven / 讚美之泉 SOP Japanese Album)、`8432`(あなたを思う 想起你 / 泥土音樂)、`8434`(恵みの道 腳步 / 泥土音樂)。**OCR 出到完整日文歌詞**,底本靚,只係 `lang` 欄錯 —— 建議改 `lang='日語'` 而唔係 unusable(舊規則會叫判死,今轉唔跟)。
4. **`3627` 同心圓《真愛》「簡單唱·澳洲特別篇」留 draft:** OCR 淨係場地 branding、whisper 10 段得 `[MUSIC]`/`[FOREIGN]`,自己救唔到;有 verified 兄弟片 `3075`(真愛 苦路十四站)但 5:56 vs 4:21 **編排唔同**,唔敢照抄。留返俾 R2/R2b 線。cantonhymnLookup 撞到嘅「真愛」係 Hillsong Real Love 粵譯,唔同歌,唔可以當核到。
5. **`8304`「稱頌祢聖名」verified 得 48 字**(4:29 嘅歌),疑似太薄,順手覆查。原本想用佢救 `8269`(台語版),0% 命中證實兩者歌詞內容唔同,已放棄,`8269` 留 draft。

### ⛔ 冇 restart —— 俾 deploy gate 擋
```
❌ abort:HEAD (fe4580b) 唔等於已批准嘅 backend.sha (94cf6de)
未批准 commit:fe4580b feat(lyrics): 複核線2條→4條,用 lang + id%2 分區
```
`fe4580b` 係 15:50 另一條 session 嘅 commit,唔關今轉事;批准權唔喺自動班,所以**冇跑 `approve.sh`,冇 restart**。今轉 30 首已寫入 DB 但**未出街**:live `/api/hymns` 抽驗 `4555`/`7805`/`6292` 三首全部仲係「冇歌詞」,live 總數 6092。DB 側已抽驗 10 首全部 `verified` + 有 lyrics,10 首 unusable 全部 `unavailable`,寫入無誤。**restart 權喺 R1 線**,佢下轉(16:45)會順手放埋出街。

**Fable 5 抽查名單(8 首):** `4470` Give Me Oil in My Lamp、`7212` 讓家偉大 English Version、`6611` 如同橄欖樹 Acoustic Live、`5742` 回到這個家、`2688` 完了(音樂分享會)、`1289` 十字路口、`6020` 最好的福氣 Saxophone 版、`6292` 最美的禮物 敬拜 MV。**請重點覆核「純靠 metadata 鎖死嗰四首」同「absolute-hit ≥7 放行」呢兩個新判準** —— 兩樣都係今轉新開,未經第二對眼。
- [2026-08-22 17:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+30**(log累計 3370);重做隊剩 0;可做draft 404;producer 行緊
- **心跳 2026-08-22 17:01** — verified 5110(83.9%)/ draft 477(可做 404)/ 重做隊剩 0 / producer 生存(paddleframe 跑緊,fetchlyrics log 17:00 仲有寫)/ 正常;keeper 上一次開班 14:26(每 2 鐘一轉,未到下一轉)

## 2026-08-22 17:2x — R1 國語線(單數 id)複核班

**時段:** 16:52 – 17:2x(用咗約 35 分鐘,3 個鐘上限用咗兩成)。**收工原因:`冇貨做`**(200 個決定上限用咗 **105**;零 rate limit 警告)。**WebSearch 用咗 0 次**(今班全部靠 DB 內部證據解決)。
...
| 🧟 判 unusable(底本死症) | **48** |
| 🗑️ 落架(非歌內容) | **5** |
| 📌 留 draft | 30(全部逐首寫低咗原因) |
| audit reject | 2(兩條 shortOk 實證唔過,冇夾硬) |
| langmismatch | **0**(7 批全部 0) |
| producer race 中招 | **0** |

全庫 `lyrics_status`(status=ok):verified **5129**、draft 408、unavailable 342、none 1153。
我開波時分區有 95 首可做 draft,收爐剩 30 首 —— **30 首全部係我今班逐首讀過、判咗「結構性做唔到」或者「等 Eric 拍板」嘅**,即係**分區真係抽乾**。

### 今班新增嘅方法論(建議收入 lyrics47-b01 SKILL 正本)

**① 開波第一件事應該係「全分區兄弟片掃描」,唔係逐首開 timeline。**
一條 SQL(`title LIKE '<純中文 key>'` + `lyrics_status='verified'`)掃晒成個分區,95 首入面即刻篩出 **33 首有兄弟片、15 首係同一首歌**。今班 17 首出街入面 **7 首**靠呢個開路。成本係一次 query,唔使開任何一首歌嘅 timeline。

**② 🔴 判 unusable 定留 draft,有一條清晰嘅分界線(今班訂,建議寫死):**
- **(A) 條片根本冇歌詞字幕** —— OCR 影到嘅淨係專輯封面水印 / setlist 標題卡 / credit 字幕 → **判 unusable**。換幾好嘅 OCR 引擎都變唔出唔存在嘅字幕,唯一出路係兄弟片;冇兄弟片就係**結構性做唔到**,唔係做得差。
- **(B) 條片有字幕但 OCR 讀爛咗** —— 讀出嚟係亂碼中文 / 斷字 / 碎行 → **留 draft**。呢類係我哋自己處理嘅問題,producer 重跑或者換引擎有機會救返,**判死就白白剷走機會**。
今班 48 首 unusable 全部屬 (A);9 首留 draft 屬 (B)。

**③ 「OCR 全水印」唔等於死症 —— 要再問多一句先可以判。**
上一轉(13:31)出咗一張「47 首天韻死症」名單,今轉實錘入面至少 **7767 人生的福、7817 永恆的答問** 兩首係**假陽性**:條片的確冇字幕,但 **whisper + 同名 verified 兄弟片兩邊夾攻就砌得返**。7817 更加係兩條獨立兄弟片(7763 + 7854)逐字一致,whisper 32 段同音錯字全部對得返(詩文=試問／銀河裡=名和利／竟找=今朝／無效=不朽／任性間=人心間)。
→ **正確次序:OCR 全水印 → 查兄弟片 → 冇兄弟片先至判 (A)。**

**④ ⚠️ 兄弟片自己都可以帶住水印污染,唔可以照抄。**
7739 福音小唱嘅兄弟片 **7974(組曲版,已 verified)** 入面有兩句係「給我真正自由 才有平安 / 有了耶穌 給我真正的自由」——「**給我真正的自由**」根本係天韻嗰隻專輯封面水印(我喺 7839 / 7753 / 7813 三首 OCR 見到同一句重複出現十幾次),被當咗歌詞寫咗入去。**7974 係雙數 id(R1b 分區)我冇郁,但嗰兩句要修。**

**⑤ 「跟自己條片定跟兄弟片」今班三次取捨,規律係:字形誤讀就改,用字風格就跟自己。**
- 改:「讚美的**輕**泉」→「**清**泉」(輕泉唔成詞)、「從**瓦**古到永遠」→「**亙**古」。
- 唔改:6543 跟自己片寫「**你**」唔跟兄弟片 5155 嘅「祢」;7061 跟自己片寫「焚燒的**靈**」(whisper 都係靈)唔跟兄弟片 6577 嘅「心」;723 跟自己片寫「**愛將**不可能變為可能」唔跟 6243 嘅「愛讓」。

**⑥ 唔好見到 whisper 覆蓋率低就留 draft —— 先數 OCR 砌出嚟嘅 CJK。**
8089 Dunamis 上轉因為「whisper 覆蓋 45%」留低,但佢 OCR 砌出嚟有 **60 CJK,本身已經過 45 門檻**,`shortOk` 同 whisper 覆蓋率根本唔關事,今轉一次過骨。

**⑦ 一條長 concert 片 = 幾首歌 × 幾條兄弟片交叉核。**
723《天堂敬拜》303 幀 OCR,先抽 unique 行(240 行)排出順序 → 分到係五首歌 → 逐首查兄弟片(5238/5295/6243/5853/2 全部已 verified)核字 → MC 講話同決志禱告全部剔走。一首出街抵過十首碎歌。

**⑧ OCR 噪音多咗一個新品種:片入面真實出現嘅第三方文字。**
8039 一公分的心跳嘅 MV 影住手機 Facebook 留言截圖同手術同意書,OCR 照讀晒(「Angela Wills 醫生説有一點小...」「手術同意書 登字號:」)。呢啲**唔係水印**(係片入面真嘢),但一樣唔係歌詞,dedupe 腳本嘅 BRAND/CREDIT regex 冚唔到,要人眼捉。

### 🔴 要 Eric 拍板嘅事(累計,今班新增粗體)

1. **`auditLyricsBatch.js` 個 shortOk 閘** —— 上轉已提「whisper 幻覺可以假過關(3772 實測)」,今轉再撞到**反方向嘅兩個假陰性**:
   - **5741 有主在我船上**(RainDrop,1:17):閘話「whisper unique 63 大過你交嘅 27,2.3×」,**但分子入面有兩段係「詞曲:李宗盛 編曲:李宗盛」幻覺 credit** —— 即係 ratio 分母污染呢個成因再一次實錘。首歌 OCR 20 幀清清楚楚、whisper 逐句對得返,係真・天然短。
   - **7961 小寶貝**(天韻,1:15):覆蓋 **84% vs 門檻 85%**,爭一個百分點;而佢根本係專輯**試聽節錄片**(片尾 OCR 有 tracklist 寫住「5. 小寶貝 2:40」)。
   → **建議一次過改三樣:(a) 計 ratio 前先剷 whisper 幻覺 credit 行、(b) 加 `whisper unique 段數 ≥ 5` 前置條件(堵上轉嗰個假陽性)、(c) 「節錄片」類個案覆蓋率門檻放寬或者改判準。**
2. **「靈修節目片」算唔算非歌內容?** —— **8417 Is there really a God?**(泥土音樂《幸福專線》,6:07,OCR 100 幀九成係廣播稿:牛頓同哈雷嘅故事、勸人讀聖經,最後先有一小段詩篇 8 嘅歌)同 **3983《沉思集》:來就上主羔羊**(新心,4 句聖詩 + 一大段 Charlotte Elliott 生平默想文)。同訪談/教學片唔完全一樣(佢哋真係夾住一首歌),所以我照 SOP 當模糊個案留 draft 冇自行落架。
3. **7695【Christmas isn't Christmas】(天韻)** —— 片頭 credit 明寫 `Words & Music: Jimmy and Carol Owens`(**在版權英文原曲**),字幕中英對照,whisper 9 段全部係「聖誕節,快樂的聖誕節」幻覺、**驗唔到原唱語言**。屬 §4 鏡像 + 英文版權暫緩兩邊嘅交界個案,我唔 apply、唔 unusable、唔落架。
4. **語言錯標(沿用上轉名單,今轉新增 5227)** —— 日語 6334/6335/5658/5666/8432、韓語 5212/5225/**5227**/5226、粵語 8178/8174。呢批 `lang` 欄寫住國語但唔係國語,`bi-freeze` 同 audit 個 langmismatch 擋板兩邊都冚唔到。
5. **id 54「好好戀愛」係世俗粵語流行曲入錯庫**(上轉已提,未處理)。

### 🎻 INSTRUMENTAL Phase 1 掃描名單(今班見到嘅純音樂,累計 4 首)
**279【QT音樂】平安篇 Quiet Time Music(天韻)**、**4159【茉莉花】讚美之泉台北青少年弦樂團**、**5981 一同起舞(讚美之泉安靜系列 1)**、5812 祢的同在(安靜系列 2,13:06 果轉已報)。
**共通點:片名/系列名已經自報家門**(`QT` / `Quiet Time` / `安靜系列` / `弦樂團`),可以直接做關鍵字掃描,唔使逐首聽。

### 🗑️ 落架 5 首(全部行咗 `--dry` 先)
**天韻音樂教室系列 3 條:8043【胸腔共鳴】、7853【學唱歌一定要找老師嗎?】、7933【母音的修飾】** —— 呢個系列成個都係發聲教學/訪談,**建議下一轉開波用 `title LIKE '%音樂教室%'` 或者「歌唱祕訣/母音/共鳴」關鍵字一次過掃埋剩低嗰啲**。
另外 2 條:**6241 讚美之泉兒童有聲書 (一)(二)**(產品宣傳:書本材質/按鍵/塗色液/售價)、**6441 2014 首屆香港兒童創意敬拜學校**(招生宣傳:開課日期/報名截止/收費/名額)。

### 串流健康
- `stream-health.log` 最後 2026-08-22 13:14 `ok=3 fail=0`,`SUPERVISION-LOG` **冇新 🔴 串流健康警報**。
- 🟢 **502 風暴收咗:最近 60 條 `[stream]`(08:13Z–09:21Z = 16:13–17:21 HKT)502 = 0 條。** 對比之前嗰 100 條有 **47 條 502**,即係今日中午之後真係好返(同 [[project-stream-outage-ytdlp-stale-2026-08-22]] 講嘅 brew 升級 yt-dlp 對得上)。**8-20 起連續三日量到嘅 31%→41%→37% 高位橫行,今次係第一次見到跌返落零。**
- ⚠️ **但最近 60 條入面有 21 條 status=404,全部係同一個 `id=2015`,而 `hymns_all` 根本冇呢個 id。** 即係有 client(大機會係 Eric 部機)攞住一個已經唔存在嘅 id **重試咗 21 次**。唔係後端問題,但**前端對 404 嘅 stream 應該唔好無限重試**,值得交前端線跟。
- [2026-08-22 18:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+29**(log累計 3399);重做隊剩 0;可做draft 357;producer 行緊

**(18:15 補充 — 404 嗰單有新料,值得交前端線跟)**
再量多 120 條 `[stream]`,404 唔止 id=2015 一個:**id=2718 撞咗 28 次、id=2015 撞咗 22 次**,另外 2420 / 1951 / 1835 各一次。**呢幾個 id 喺 `hymns_all` 完全唔存在**(唔係 `status='rejected'` 落架 —— 落架嗰啲仲喺表入面),即係**用戶部機嘅清單/隊列入面揸住一批已經冇咗嘅 id**,而 client 見到 404 之後**照樣重試幾十次**。
→ 兩件事建議跟:(1) 前端 `/api/stream/<id>` 收到 **404 應該即刻放棄兼跳去下一首**,唔好重試(404 同 502/403 唔同,重試永遠唔會成功);(2) 清單同步/開 App 嗰陣應該**對一次 live 歌單剷走死 id**,唔好留喺本地清單度。

**(18:38 收爐補充 — 班尾 checkpoint 結果)**
- **§5.1 靜音等咗 77 分鐘先過**(17:21 開始量,10:36:41Z 先至靜足 21 分 42 秒)。第一輪等到 20 分鐘,**腳本收工後 6 秒就有新播放**,所以冇 restart,改用 21 分鐘 buffer 再等 —— 呢個 6 秒 race 值得記低,**下次直接用 21 分鐘門檻**。
- **§5.2 approve 撞到 HEAD 中途郁咗:**18:16 前置檢查嗰陣 approved=`94cf6de` / HEAD=`36fccbd`;18:36 真正 approve 嗰陣 approved 已經係 `2f4c26b`、HEAD 已經係 `1100324`。另一條線落咗 **`1768a5b` fix(player)「(已下架)」佔位項唔再餵落 TrackPlayer** 同 **`2f4c26b` fix(sync) `/api/me` 加 hymn_id 存在性驗證 + `reconcileUserRefs.js`** —— **即係我 18:15 喺呢度報嗰單「死 id 撞 404 幾十次」嘅修復,對方已經自己行過 gate 批准埋**。唔屬「唔明嘅 backend 改動」,所以照 approve HEAD。
- **§5.3 restart:今轉唔使做。** `ps -o lstart=` 顯示 backend process 開機時間 `Sat Aug 22 18:14:37`,喺 `2f4c26b`(18:09:56)之後 —— **另一條線啱啱重啟過,順手已經載入咗我 17:19 之前寫入 DB 嘅全部歌詞**,冇必要再打斷佢一次。`launchctl list | grep hymnapp | wc -l` = **7** ✓。
  → 📌 **方法論:「另一條線啱啱 restart 過」係一個要主動查嘅狀態**,唔好見到自己有 apply 就條件反射 restart。一條 `ps -o lstart= -p $(pgrep -f "node.*server.js")` 對比 approved commit 時間就分到。
- **§5.4 live 驗證照做足:**`/api/hymns` 6082 首,**今班 17 首逐首查過 17/17 全部有歌詞、零漏**;5 首落架已唔喺 live;**48 首 unusable 仲喺 live 播得,歌詞字數 = 0** —— 實證咗 `unusable` 只係踢出複核隊列,唔影響播放。
- [2026-08-22 19:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+35**(log累計 3434);重做隊剩 0;可做draft 379;producer 行緊
- [2026-08-22 20:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+16**(log累計 3450);重做隊剩 0;可做draft 274;producer 冇行
- **心跳 2026-08-22 20:01** — verified 5164(85.1%)/ draft 351(可做 274)/ 重做隊剩 0 / producer 生存 / OCR 池 118 首全部撞 12 鐘頭 cooldown 或 skip-orgs,連續兩轉零攻,keeper 已自動轉 CC 補倉
- [2026-08-22 21:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+0**(log累計 3450);重做隊剩 0;可做draft 272;producer 冇行
- [2026-08-22 22:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+0**(log累計 3450);重做隊剩 0;可做draft 212;producer 冇行

---

## 🤖 每日自動歌詞校對 routine — 2026-08-22 21:43–22:02

**分區(避開行緊嘅線):** 開波先數過兩條 active 線 —— `b0283212`(R1 國語)同 `393c1591`(R2 粵語)嘅 scratchpad batch **75 個 id 全部係單數**,所以今轉揀 **雙數 id、非英文** 做分區(92 首)。英文 126 首照 `backend/data/lyrics-copyright-hold.json`(Eric 2026-08-19 拍板「唔 apply、唔判 unusable、唔 delist」)**完全冇掂**。

**現況數字(收爐時,全庫 curated 6052 首):** verified **5183(85.6%)** / draft 282 / unavailable 457 / 未做 130。
draft 282 拆開 = 英文 hold 126 + 雙數非英文 78(我分區,由 92 減落嚟) + 單數非英文 78(R1/R2 線)。

**今轉產出:verified 4 / unusable 10 / 留 draft 78 / reject 0。**
- `auditLyricsBatch.js` 三個 batch 全部 **passed,零 reject、零 langmismatch**。
- 三次 `reviewLyrics.js --apply` 全部成功(4 verified + 10 unusable)。
- restart 行咗 `ops/deploy/backend-restart.sh`(**唔係** task file 寫嗰個 `launchctl kickstart` —— 跟 HANDOFF §2.10 嘅唯一合法路徑),gate 全過(HEAD == approved `29fe63f`)。
- **restart 前確認過靜音 47 分鐘**(最後一條 `[stream]` 係 13:11Z,restart 喺 13:59Z),冇打斷任何人聽歌。

### 🎯 Fable 5 抽查名單(今日 verify 嘅只有 4 首,全列)
| id | 歌名 | 來源兄弟片 | 證據 |
|---|---|---|---|
| 7132 | 約書亞樂團 -【主是我的拯救 / Lord, My Savior】Live | 7271 | 自己 OCR 命中兄弟片 16/17 行(0.94),唯一漏嗰行係斷行差異 |
| 7310 | 【我永遠相信 / I'll Always Believe】Music Video | 7270 | **5 條獨立 verified 兄弟片**內容一致,自己 OCR 命中 7/9 |
| 3054 | 新歌試唱 :: HOPE音樂會《說有就有》+《詩90》 | 3053 + 3050 | 逐幀定序實錘:24–120s 全部命中 3053、292–464s 全部命中 3050,**中間 120–292s 空白 = 兩首歌之間講話** |
| 7572 | This Is Living kuso版 | 7563 | 命中 16/22,漏嗰 6 行喺自己 OCR 逐句搵得返(斷行唔同) |

### 🔬 今轉方法論(三招,建議寫入常規)

**① 兄弟片配對唔應該靠歌名 —— 改用「全庫 verified 行級反查索引」,一轉由 2 首變 4 首。**
之前兩轉都係「同 artist + 同名 + duration≤3s」搵兄弟。今轉實測**歌名正規化本身就係最大失敗源**:剝走【】括號會令「【恩典與我同行】Music Video - 約書亞樂團」變成 `約書亞樂團`,一條 draft 配到成個頻道 **22 條假兄弟**。
改咗做法:將全庫 5183 首 verified 嘅**每一行歌詞**(去標點、≥6 字)頭 6 個字入 index,再攞 draft 自己嘅 OCR+whisper 池**逐個 6 字窗口反查**。歌名完全唔入方程式。
- 效果:92 首 draft 入面 **16 首**搵到 hit≥5 嘅候選(歌名法只有 12),而且捉到歌名法**永遠捉唔到**嗰隻 —— `7572「This Is Living kuso版」` ← `7563「這就是活著」`,兩個歌名冇一隻字相同。
- 成本:全庫 index 一次過砌,92 首掃完幾秒。

**② 「兄弟片漏咗 N 行」要分開睇係「真係冇唱」定「斷行唔同」—— 唔查清楚會白白扔咗合格個案。**
`7572` 表面命中 16/22(0.73),差啲照 <0.75 掉。逐行揪出嗰 6 行嚟睇,發現全部係**兄弟片一行 = 自己 OCR 兩行**(例:兄弟寫「看陽光穿透烏雲…」9 字一行,自己 OCR 斷開兩截),實際覆蓋接近滿分。
→ **建議:整數字門檻(hit / ratio)之後一定要行多一步「miss 行逐個 substring 再查」**,先好落判斷。同一招亦令 `7310` 由「差一行」變「確認得一行係 OCR 真係漏咗,而 5 條兄弟片全部有」。

**③ 反過嚟:命中率低就要即刻收手 —— `6650` 差啲被 hit=7 呢個絕對數騙咗。**
上轉方法論寫住「絕對命中行數 ≥7 比百分比可靠」。今轉 `6650【祢是我的光】` 啱啱好 hit=7,但 ratio 得 **0.28(7/25)**,而佢自己 OCR 有 39 個 block、3029 字,唔係「資料唔夠」。逐行查 miss:**18 行係一整段完全唔同嘅內容**(兄弟片 6559/6655 係 6:38 嘅「＋甦醒」組曲版,自己得 3:22)。
→ **判準修正:hit≥7 係必要條件唔係充分條件。當自己 OCR 夠厚(>2000 字)但 ratio <0.5,基本上實係「兄弟片係組曲/長版」,一定要留 draft。**

### 🪦 unusable 10 首(全部 `lyrics_draft` 保留,可逆)
判準:**OCR 得標題卡/水印/亂碼 → 查全庫兄弟片(零命中) → whisper unique ≤4 或者全幻覺 → 先至判死。**

| id | 歌名 | 死因 |
|---|---|---|
| 3634 | 同心圓《To Love》TWS | OCR 得 1 個 block(全係音樂會 branding),whisper 3 段係 `[singing in Cantonese]` 佔位符 |
| 2690 | 耶和華以勒 // 阿修(悅雨 GRM) | OCR 4 個 block 全部係場地/教會水印,whisper 3 段英文幻覺 |
| 4164 | 【望春風】讚美之泉台北青少年弦樂團 | **純器樂**(弦樂四重奏台灣民謠),OCR 全係指揮/四重奏 credit,whisper 2 段幻覺 |
| 5804 | 【展開清晨的翅膀】讚美之泉安靜系列 (2) | **純器樂**,OCR 60 個 block 全部係專輯名「找一個地方」重複亂碼,whisper `[MUSIC]` |
| 6348 | 【信じれば奇跡は起こる】日語兒童 MV | 日語內容標咗國語;OCR 7 個 block 全 credit(中文得 5 個字),whisper 只有羅馬拼音 |
| 5212 | 【주 다스리소서】韓語 | **whisper 0 段**,OCR 28 個 block 全部韓文亂碼 |
| 5226 | 【생수의 강물】韓語 | OCR 93 個 block 全部韓文亂碼(「刘早全人」咁),whisper 3 段全部係「詞曲:李宗盛」幻覺 |
| 3552 | Rain Down - 福臨敬拜隊 | OCR 15 個 block 一半 UCLONE 水印,真歌詞得 2 句喺度重複,whisper 3 段幻覺 |
| 2768 | AKF - 這是什麼道理(悅雨音樂盒) | OCR 一半係「講員專用」水印,真歌詞得 5–6 句,whisper 1 段幻覺 |
| 4742 | 【廣東話兒童詩歌】《神愛你和我》 | OCR 11 個 block 得 branding + 「God loves you and me」重複,中文歌詞近乎零 |

### ⚠️ 今轉新發現 / 要跟嘅事

**1. `ocrFrames` 少 ≠ OCR 抽幀失敗 —— 我一度判斷錯,查完 `lib/ocrMerge.js` 先糾正返。**
`lyrics_timeline.ocr` 入面嗰個 array **唔係逐幀**,係 `mergeOcrLines()` **剷走水印之後再按相似度合併**嘅 block。所以「得 2 個 block」係「成條片得兩段唔同文字」,係**冇字幕嘅證據**,唔係抽幀壞咗。呢個誤解差啲令我將 10 首判死改成「留 draft 等重做 OCR」。**建議寫入常規判準,唔好再撞。**

**2. 判死前嘅「whisper 例外」今轉救返 2 首,建議正式寫入 SOP。**
判準 A(OCR 全水印 = unusable)喺 `7718 你我歌`(天韻)同 `7966 欣賞`(天韻)兩首**唔應該照用**:OCR 的確 100% 係「天韻創作專輯之十」水印變體/tracklist 目錄卡,但 whisper 分別有 **10 段同 4 段真實非幻覺歌詞**(唔係 `[MUSIC]`、唔係「詞曲:李宗盛」)。呢兩首係「人手可救」唔係「死症」,已經留 draft。
→ **SOP 建議:`OCR 全水印` + `whisper unique ≥5 而且內容唔係佔位符/幻覺 credit` = 留 draft,唔准判 unusable。**

**3. 🔴 自動 routine 出唔到「人手重組」嘅 verified —— 呢個限制今轉再一次實錘,建議 Eric 知悉。**
我行嘅守則唔准重現在版權歌詞正文,所以**凡係要人手讀 OCR 再打返出嚟嘅個案,自動 routine 一首都做唔到**。今轉 4 首 verified **全部**係用 script 由我哋自己 DB 嘅兄弟片搬字(script 讀 DB → 寫 apply 檔,全程冇經過我打字),所以合規。
→ 即係話:**自動班嘅實際產能上限 ≈ 「有兄弟片可搬」嗰批**。今轉 92 首入面得 4 首合資格(4.3%)。剩低 78 首入面起碼 **30 首係 OCR 質素好、人手一定救得返**(例:`3744 榮耀神羔羊`、`3582 Hosanna 廣東話版`、`7178 Let's go`、`6574 我願意為祢`、`7980 Go Tell It on the Mountain`、`3014 只等你願意` —— 全部中英對照字幕齊全),**淨係等人手線嚟做**。

**4. 語言錯標 —— 今轉新增 6 首(累計名單再長)。** 呢批我**冇判死**(佢哋底本一流,死咗好嘥),但下一轉 export 會再見到佢哋:
- **日語標咗國語:** `8428 愛を形に`、`8432 あなたを思う`、`8434 恵みの道`(泥土音樂盛曉玫日語版三首,**OCR 日文字幕相當靚**,純粹 `lang` 欄錯)
- **英文標咗粵語:** `3938 Brand New Day`(Milk&Honey)、`5352 Still Alive`(HKACM)、`6128 I Love Sunshine`(May姐姐) —— 三首都係英文原創,`lang` 寫粵語,所以**避開咗英文版權 hold 個擋板**,會一路喺粵語隊列度轉
- **粵語標咗國語:** `8174 因主歡欣 (粵語)｜小羊詩歌`
→ 連同之前幾轉報過嗰批(6334/6335/5658/5666/8432/5212/5225/5226/5227/8178/8174),**建議開一次過嘅 `lang` 修正 script**,唔好再一首首喺 log 度累積。

**5. 非歌內容 / 合輯 3 首(我冇 delist 權,淨係報)** —— routine 紅線寫明「DB 寫入只准經 `reviewLyrics.js --apply`」,而 delist 要行 `lib/adminHymns.js`,所以今轉冇郁:
- **`1644《兩個世界．上下城》歌舞劇原聲帶`(台北復興堂)** —— 成隻音樂劇原聲帶,OCR 343 個 block、3106 個中文字,明顯係多首歌合輯
- **`722 那些年,我們一起唱的歌 ||: 除祢以外 / 注目看耶穌 / 全新的你 / 雲上太陽 / 禱告 / 寶…`(讚美之泉)** —— 標題自報家門嘅七首以上組曲
- **`54 好好戀愛`(方力申 + 鄧麗欣)** —— **世俗粵語流行曲入錯庫**,OCR 睇到「男：/女：」對唱標記同完整流行曲歌詞。⚠️ **呢首係第五次喺 log 出現仍未處理。**

**6. 純音樂 Phase 1 掃描名單新增 2 首:** `4164【望春風】讚美之泉台北青少年弦樂團`、`5804【展開清晨的翅膀】讚美之泉安靜系列 (2)`。
兩首都印證咗上轉嗰個觀察 —— **片名已經自報家門**(`弦樂團` / `安靜系列`)。連同之前報過嘅 279 / 4159 / 5981 / 5812,`安靜系列` 同 `青少年弦樂團` 兩個關鍵字已經出過 **6 首**,值得直接 keyword 掃全庫。

**7. 串流健康:** `stream-health.log` 最後 2026-08-22 20:18 `ok=3 fail=0 mid=3 midfail=0`,冇新警報。restart 前後 health check 都過。
- [2026-08-22 23:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+2**(log累計 3452);重做隊剩 0;可做draft 213;producer 冇行
- **心跳 2026-08-22 23:01** — verified 5183(85.6%)/ draft 283(可做 213)/ 重做隊剩 0 / producer 生存(pid 28362)/ OCR 池 118 首仍然全部撞 cooldown / skip-orgs,由 20:01 到而家連續零攻已第三個鐘,keeper 每轉自動轉 CC 補倉但 /tmp/hymn_fetchlyrics.log 最後寫入停咗喺 15:01(等於 producer 實際成 8 個鐘冇出過新 draft),draft 池 351→283 淨係跌冇補
- [2026-08-23 00:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+0**(log累計 3452);重做隊剩 0;可做draft 201;producer 冇行
- [2026-08-23 01:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+0**(log累計 3452);重做隊剩 0;可做draft 199;producer 冇行
- [2026-08-23 02:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+0**(log累計 3452);重做隊剩 0;可做draft 194;producer 冇行
- **心跳 2026-08-23 02:01** — verified 5192(85.8%)/ draft 263(可做 194)/ 重做隊剩 0 / producer 生存 / OCR 池 118 首全部撞 12 鐘頭 cooldown 或 skip-orgs,連續兩轉零產出(keeper 已自動轉 CC 補倉),draft 淨增長靠人手複核線
- [2026-08-23 03:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+0**(log累計 3452);重做隊剩 0;可做draft 193;producer 冇行
- [2026-08-23 04:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+0**(log累計 3452);重做隊剩 0;可做draft 191;producer 冇行
- [2026-08-23 05:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+0**(log累計 3452);重做隊剩 0;可做draft 191;producer 冇行
- **心跳 2026-08-23 05:01** — verified 5194(85.8%)/ draft 260(可做 191)/ 重做隊剩 0 / producer 生存 / OCR 池 118 首仍然全部撞 cooldown / skip-orgs,已連續四轉零產出(04:51、04:56 兩轉都係「冇一首攻得」),自動線實質空轉;三個鐘 verified 只 +2,全靠人手複核線
- [2026-08-23 06:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+0**(log累計 3452);重做隊剩 0;可做draft 190;producer 冇行
- [2026-08-23 07:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+0**(log累計 3452);重做隊剩 0;可做draft 190;producer 冇行
- **心跳 2026-08-23 08:01** — verified 5196(85.9%)/ draft 276(可做 186)/ 重做隊剩 0 / producer 生存 / keeper 07:31 報「上一轉 OCR 池全部 cooldown / skip-orgs 攻唔到」被迫轉 CC 補倉,但 08:01 OCR job(budget 120)行緊 paddle 抽幀,暫時唔算斷路
- [2026-08-23 08:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+23**(log累計 3475);重做隊剩 0;可做draft 209;producer 行緊
- [2026-08-23 09:01] P線時報(keeper自動):過去1小時 OCR/whisper draft **+34**(log累計 3509);重做隊剩 0;可做draft 242;producer 行緊

## 2026-08-23 09:30 — `lyrics-daily-proofread` 已 pause(Eric 拍板)

**原因:** 8/22 複核線由 2 條加到 4 條(R1/R1b 國語單雙、R2/R2b 粵語單雙),四條線用 `lang` + `id % 2` 硬分區、零重疊。但 `lyrics-daily-proofread`(每日 09:40/15:40/21:40)係舊嘅**通用** routine,**唔認識呢個分區機制**,會攞晒全部 draft 嚟做 → 隨時同四條專線任何一條撞單,兩邊做同一首歌。

後果係**浪費唔係破壞**(apply 冪等,唔會整壞數據),但兩條線嗰陣撞機會細、四條線之後就大幅上升,所以 pause。

**而家跑緊嘅複核線(4 條,每個鐘平均有一條開波):**
| 線 | 分區 | cron |
|---|---|---|
| R1 | 國語 單數 id | `45 12,16,20,0,4,8` ✅ 唯一 restart owner |
| R1b | 國語 雙數 id | `45 14,18,22,2,6,10` |
| R2 | 粵語 單數 id | `0 13,17,21,1,5,9` |
| R2b | 粵語 雙數 id | `0 15,19,23,3,7,11` |

**停咗嘅:** `lyrics-daily-proofread`(今次)、`lyrics-line-english`(8/19 版權暫緩)、`lyrics-catchup`(8/22 已被四線取代)。
**仲跑緊嘅非複核 job:** `lyrics-progress-heartbeat`(每 3 鐘寫一行進度,唔複核)、`com.hymnstream.healthcheck`(每 6 鐘串流探測)。
