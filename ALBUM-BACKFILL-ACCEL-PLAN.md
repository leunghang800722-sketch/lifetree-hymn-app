...
## 9. 執行記錄(2026-08-17 第二輪 —— 收殘餘)

接住 §8 嘅「下輪剩低」逐項做。**寫入 8 首**(全部係 curated 活 row,
1573 → 1565),但**真正價值喺三個新資料源同兩個新擋板**。

### 9.1 CantonHymn:推翻咗「approve 個 playlist」呢條路,但搵到更好嘅嘢

原計劃係 approve `CantonHymn-playlists.json` 嗰個 64-member playlist
(matched 8 首)。**實際爬完 64 條片對返 DB:只有 10 條在庫,其中 9 條
已經有 album,得 1 條冇。**而且個 `proposed_album` 係
「恢復粵語敬拜共建專輯系列」——**呢個係系列名唔係專輯名**,填落去係錯。
**所以冇 approve,冇用個 pipeline。**

改為:由 playlist 入面幾條「全碟試聽」片嘅 **description 逐曲清單**起咗
`cantonhymn-cobuild-catalog.json`(37 首 / 4 隻碟):
- 共建專輯(四)HKWorship I(2019)—— 14 首完整曲序 + 每首嘅演出機構
- 共建專輯(五)陪我走過春夏秋冬(2020)—— 12 首完整曲序 + 演出機構
- 共建專輯(二)同心合意(2017)、(一)合一的呼求(2015)—— 只攞到部分,
  catalog 標 `partial:true`
- 共建專輯(三)「愛濤」—— 搵極都冇任何逐曲清單,冇收錄

**關鍵洞察(之前漏咗嘅):呢個系列係合輯,每首歌掛喺唔同機構名下**
(角聲使團 / HKACM / Milk&Honey / 原始和聲 / 鹹蛋 / Son Music / 小羊詩歌…)。
即係話呢批 album 資料**唔應該淨係對 org='CantonHymn' 去搵**,要跨 org 對全庫。
`backfillAlbumFromCobuildCatalog.js` 就係咁做。**寫入 3 首。**

> ⚠️ **踩過一個大坑,記住**:第一版用 `title.startsWith(歌名)` 出咗 28 個
> 候選,**人手一睇 22 個係垃圾**——呢個合輯好多歌名係極通用詞(「一」
> 「使命」「尋找」「呼喚」「傾倒」「憐憫」),「一」直接撞中「一生奉獻」
> 「一公分的心跳」「一位母親的付出」…514 首。收緊咗兩重先啱:
> ① 歌名要係**完整 token**(前後都係開頭/結尾或者分隔符),短過 3 個字
>    就淨准全標題完全相等;
> ② 仲要**演出單位對得上**(catalog 有 performer 欄,對 DB 個 org)。
>    冇咗呢重,KEC 自己 cover 嘅〈傾倒〉會被當成專輯(二)嗰首讚美之泉授權版。
>
> **驗證**:收緊之後 6 條「DB 已有 album」嘅 row 入面 4 條同 catalog 完全
> 一致(陪我走過春夏秋冬),另外 2 條唔一致但**係 DB 啱**(〈來充滿我〉
> DB 填《活祭》原碟、catalog 係粵語精選集)——保護規則冇覆寫,啱。

### 9.2 天韻:10 個解唔到嘅 conflict → 剩返 1 個

兩個修正:
1. **年份 regex 太窄**——`fetchTianyunAlbumYears.js` 本來只捉「YYYY年M月發行」,
   但 shop.hms.org.tw 唔止一種寫法:《飛翔》係「2000年3月**出版**」。
   加咗 `發行|出版|上市|推出|問世`,53 隻碟由 38 → 39 隻有年份。
   《飛翔》=2000 一補返,〈眼光〉〈浪子〉即刻解到(伊的疼惜 2002 vs 飛翔 2000)。
2. **新擋板:同年平手 → 睇 DB 內部先例**。《不為明天憂慮》(國語)同
   《莫得為明日掛慮》(台語)係**同一張碟嘅兩個語言版**——1993 同年、
   13 首曲序完全一樣、其中 6 首曲名一模一樣,靠年份永遠分唔到。但早期
   'search' 輪已經幫〈心中得安息〉〈得救者之歌〉填咗國語嗰隻。
   而家平手就跟返「同 org、同一首歌嘅其他 row 填咗邊隻」呢個先例。

**10 → 1**(剩返〈主來的日子近了〉,兩個版本都冇先例)。**寫入 2 首。**
仲有 14 隻碟冇年份,但全部係近年單曲/USB/英文碟,商品頁本身冇寫發行日。

### 9.3 Milk&Honey「More than a Concert 2015 LIVE」:**唔係專輯,唔寫**

§8 留低嗰 9 首,查完實錘唔係 release:
- 片尾 description 白紙黑字:「Milk&Honey **第一次音樂分享會** More than
  a Concert **回顧**」(2015-09-13 上載)
- 曲目大部分係**世俗流行歌 cover**:自知之明(謝安琪)、Patience(Take
  That)、Fix You(Coldplay)、飛(Supper Moment)、陀飛輪(陳奕迅)、
  幸福摩天輪——敬拜事工唔會出一隻咁嘅碟
- 片名編號 01-09 係**分集上載**,唔係碟嘅曲序

結論:一場音樂分享會嘅片段回顧。**冇寫 DB。**

### 9.4 讚美之泉:搵到第二個資料源(呢輪最有價值嘅發現)

**原本個 `sop-catalog.json` 係爬 `sop.org/copyright-ccli/`——嗰個係 CCLI
版權登記表,只列有登記版權嘅歌,結構上會漏嘢。** 官網
`sop.org/music/` 先係真正 discography:
...
多出嘅 9 隻包括:安靜敬拜/靈修系列(《住在祢裡面》《能不能》《每一天我
需要祢》)、國際版、日韓版(《예수, 내 모든 것》《天が開き》)、
《恩典之路》《深深愛祢》《差遣我》《單單只為祢》等。

新寫 `fetchSopSiteCatalog.js` + `backfillAlbumFromSopSiteCatalog.js`,
出 `sop-site-catalog.json`。**寫入 3 首。**

> **順帶做咗一次方法論驗證(重要)**:呢輪有 840 條 row「DB 已有 album」,
> 啱好可以攞嚟核對。抽報告頭 60 條:**57 條同 catalog 完全一致(95%)**,
> 3 條唔一致全部係命名差異(《我相信》vs《I Believe》同一隻碟嘅中英文名、
> 《我能給你什麼？》多個問號)。即係話 §8 嗰條「最早發行=原碟」規則
> **喺一個獨立資料源上面得到 95% 交叉驗證**。
>
> ⚠️ 又踩多一個坑:第一版 4 個候選有 1 個係錯——
> 「2019 讚美之泉敬拜讚美專輯 (23&24) 平安・I Believe **[我相信] 宣傳短片**」
> 個 matcher 由中括號抽咗「我相信」再 map 去 2011 年隻碟。加咗
> `NON_SONG_RE` 擋板(宣傳短片/預告/幕後/花絮/開箱/訪談/異象影片/NG畫面),
> 呢類 row 一律唔准 match——**佢哋標題必然引用到歌名,係 false positive 溫床**。

### 9.5 約書亞:第二個角度都試過,結論不變

- **官網 catalog 唔係過時**:`joshua-catalog.json` 59 隻碟,最新到 2026
  (《卸下冠冕》《如同橄欖樹 Acoustic Live》),2024-2026 嘅碟全部有。
- **MusicBrainz 反而更少**:約書亞樂團 MBID
  `9896dc8f-e5ae-4af1-87c8-a82d2f14544b` 得 135 個 recording(官網 696 首),
  而且全部係舊碟(真到世界盡頭/抓住永恆/超自然/榮美輝煌/觸摸到你),
  補唔到嘢。
- 逐首查 9 首韓團合作歌(我唯一渴望/等候祢/主我神/呼喚著我名/愛無止盡/
  不再有黑夜/即或不然的生命/神的光中/祢是我的光)——**冇一首喺任何一隻
  碟入面**。

結論維持:**呢批係 約書亞 × 韓國敬拜團(J-US 제이어스 / Yeram Worship)
合作嘅獨立數位單曲,唔屬於任何專輯。** 唔係資料源攞漏。

### 9.6 呢輪寫入

| 做法 | 寫入 |
|---|---|
| CantonHymn 共建專輯合輯 catalog(跨 org) | 3 |
| 天韻:年份 regex 加闊 + 同年平手睇 DB 先例 | 2 |
| 讚美之泉官網 /music/ catalog(第二資料源) | 3 |
| **合計** | **8** |

全庫 curated album 覆蓋 **75.5%**(4816/6381)。

### 9.7 ACM(38)同小羊詩歌(16):兩個資料源都試過,冇得再補

呢兩隊之前冇正式覆查過殘餘,今輪補做:

**ACM —— 官網 eshop 係新資料源,但補唔到嗰 38 首**
- `www.hkacm.org/eshop/` 有 44 個 product,其中約 11 個係專輯,而且**產品頁
  真係有逐曲清單**(例:《站立得穩》4 首、40 周年《REBIRTH》11 首)。
  即係話呢條路技術上行得通。
- **但係**:eshop 只有近年嘅碟,同 `acm-catalog.json`(YT playlist id-exact,
  20 隻碟 182 首)**大幅重疊**。而 38 首殘餘逐首查——
  祢是配得受讚揚 / 今我願意 / 跟隨祂 / 有耶穌不用怕 / 童夢 / Hosanna /
  同證福音 / 幸福藍圖 / 存在 / 凡人 / 開心小門徒 / 能見道 / 走出幽谷 /
  付愛 ——**冇一首喺 acm-catalog 或者 eshop 任何一隻碟入面**。
- 「齊唱敬拜精選100USB」同「敬拜讚美詩歌合訂本系列一/二」呢啲合輯產品頁
  **冇列曲目**(試過,numbered lines = 0),補唔到舊碟。
- 殘餘性質:詩歌創作大賽 2015 參賽作品、KTV 版、籌款音樂會精華片段,加上
  一批冇被放入任何專輯 playlist 嘅零散上載。**冇夾硬填。**

**小羊詩歌 —— 16 首殘餘全部係非專輯內容**
【中英字幕】翻譯詩歌、【禱告音樂】EnterCalm《寧靜時分》系列、
【靈修短視頻】、楊航個人作品、〈武漢我為妳祈禱〉Live、〈小羊事工創立簡史〉。
另外 `xiaoyang-catalog.json` **冇 year 欄**,所以「最早發行=原碟」規則喺
呢隊用唔到(不過佢本身淨係 4 個 conflict,影響好細)。

### 9.8 呢日埋單 + 之後仲可以做咩

**兩輪合共寫入 107 首**(§8 99 首 + §9 8 首),全庫 curated album 覆蓋
**75.5%**(4816/6381)。

**已經逐隊查證、確認冇得再補嘅 org**(每隊都至少對過兩個獨立資料源):
CantonHymn 264 / 悅雨音樂 GRM 138 / 天韻 126 / 611 Worship 116 /
讚美之泉 85 / 約書亞 81 / KEC 66 / Giggles and Tunes 57 / Milk&Honey 50 /
CJ and Friends 45 / 原始和聲 44 / 同心圓 43 / 新心 43 / 我心旋律 38 /
ACM 38 / 小羊詩歌 16。

**真係仲有得做嘅,得返呢幾樣(全部係細數目)**:
1. 天韻〈主來的日子近了〉1 首 —— 國語/台語同碟兩版平手,DB 又冇先例。
   要人手聽下條片係邊個版本先分到。
2. 天韻 14 隻碟冇年份 —— 商品頁本身冇寫發行日(近年單曲/USB/英文碟),
   除非搵第二個源,否則補唔到。
3. CantonHymn 共建專輯(三)「愛濤」—— 搵極冇逐曲清單。第日搵到就直接
   加入 `cantonhymn-cobuild-catalog.json`,個 backfill script 會自動命中。
4. `cantonhymn-cobuild-catalog.json` 37 首之中大部分**未入庫**——第日
   growLibrary 收到呢啲歌,再跑一次個 script 就會自動補到 album。

**方法論結論**:呢兩輪之後,「起新 catalog」同「解舊 conflict」兩條路
都榨得差唔多。剩低嘅 1565 首冇 album,**絕大部分係結構性冇專輯嘅內容**
(demo / 單曲 / 訪談 / 教學 / 花絮 / 靈修短片 / 系列節目 / 比賽作品),
唔係資料源搵漏。下次如果要再推高覆蓋率,應該係等 growLibrary 收新歌
之後再跑現有 script,而唔係再開新 org 嚟爬。
